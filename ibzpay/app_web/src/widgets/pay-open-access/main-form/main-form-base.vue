<template>
    <i-form :model="this.data" class='app-form' ref='form'  id='payopenaccess_main' style="">
    <input style="display:none;" />
    <row >
            
<i-col v-show="detailsModel.accessname.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-item name='accessname' :itemRules="this.rules().accessname" class='' :caption="$t('entities.payopenaccess.main_form.details.accessname')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.accessname.error" :isEmptyCaption="false" labelPos="LEFT">
    <input-box 
...
<i-col v-show="detailsModel.open_type.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-item name='open_type' :itemRules="this.rules().open_type" class='' :caption="$t('entities.payopenaccess.main_form.details.open_type')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.open_type.error" :isEmptyCaption="false" labelPos="LEFT">
    
 <dropdown-list 
    v-model="data.open_type" 
    :data="data" 
    :context="context"
...
<i-col v-show="detailsModel.access_key.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-item name='access_key' :itemRules="this.rules().access_key" class='' :caption="$t('entities.payopenaccess.main_form.details.access_key')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.access_key.error" :isEmptyCaption="false" labelPos="LEFT">
    <input-box 
    v-model="data.access_key"  
    @enter="onEnter($event)"  
     unit=""  
    :disabled="detailsModel.access_key.disabled" 
    type='text' 
    style="">
</input-box>
...
<i-col v-show="detailsModel.secret_key.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-item name='secret_key' :itemRules="this.rules().secret_key" class='' :caption="$t('entities.payopenaccess.main_form.details.secret_key')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.secret_key.error" :isEmptyCaption="false" labelPos="LEFT">
    <input-box 
    v-model="data.secret_key"  
    @enter="onEnter($event)"  
     unit=""  
    :disabled="detailsModel.secret_key.disabled" 
    type='text' 
    style="">
</input-box>
...
<i-col v-show="detailsModel.region_id.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-item name='region_id' :itemRules="this.rules().region_id" class='' :caption="$t('entities.payopenaccess.main_form.details.region_id')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.region_id.error" :isEmptyCaption="false" labelPos="LEFT">
    <input-box 
...
<i-col v-show="detailsModel.access_token.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-item name='access_token' :itemRules="this.rules().access_token" class='' :caption="$t('entities.payopenaccess.main_form.details.access_token')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.access_token.error" :isEmptyCaption="false" labelPos="LEFT">
    <input-box 
...
<i-col v-show="detailsModel.expires_time.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-item name='expires_time' :itemRules="this.rules().expires_time" class='' :caption="$t('entities.payopenaccess.main_form.details.expires_time')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.expires_time.error" :isEmptyCaption="false" labelPos="LEFT">
...
<i-col v-show="detailsModel.redirect_uri.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-item name='redirect_uri' :itemRules="this.rules().redirect_uri" class='' :caption="$t('entities.payopenaccess.main_form.details.redirect_uri')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.redirect_uri.error" :isEmptyCaption="false" labelPos="LEFT">
    <input-box 
...
export default class MainBase extends Vue implements ControlInterface {
    ...
    return {
        ...
        access_token: [
            ...
            { type: 'string', message: '管理账号token 值必须为字符串类型', trigger: 'blur' },
            { required: false, type: 'string', message: '管理账号token 值不能为空', trigger: 'change' },
            { required: false, type: 'string', message: '管理账号token 值不能为空', trigger: 'blur' },
        ],
        expires_time: [
            { type: 'string', message: '管理账号token过期时间 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '管理账号token过期时间 值必须为字符串类型', trigger: 'blur' },
            { required: false, type: 'string', message: '管理账号token过期时间 值不能为空', trigger: 'change' },
            { required: false, type: 'string', message: '管理账号token过期时间 值不能为空', trigger: 'blur' },
        ],
        redirect_uri: [
...
, 
        expires_time: new FormItemModel({ caption: '管理账号token过期时间', detailType: 'FORMITEM', name: 'expires_time', visible: true, isShowCaption: true, form: this, isControlledContent: false , disabled: false, enableCond: 3 })
, 
...
, 
     ...
     * @memberof MainBase
     */
    @Watch('data.access_key')
    onAccess_keyChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'access_key', newVal: newVal, oldVal: oldVal });
    }
     ...
     * @memberof MainBase
     */
    @Watch('data.secret_key')
    onSecret_keyChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'secret_key', newVal: newVal, oldVal: oldVal });
    }
     ...
     * @memberof MainBase
     */
    @Watch('data.region_id')
    onRegion_idChange(newVal: any, oldVal: any) {
    ...
    /**
     * 监控表单属性 redirect_uri 值
     *
     * @param {*} newVal
     * @param {*} oldVal
     * @memberof MainBase
     */
    @Watch('data.redirect_uri')
    onRedirect_uriChange(newVal: any, oldVal: any) {
    ...
    public onFormLoad(data: any = {},action:string): void {
        ...
        // 更新context的实体主键
        if(data.payopenaccess){
            Object.assign(this.context,{payopenaccess:data.payopenaccess})
        }
    ...
    public print(){
        let _this:any = this;
        _this.$print({id:'payopenaccess_main',popTitle:'主编辑表单'});
    }
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'PayOpenAccessEditView' + (this.$t('app.formpage.notconfig.loadaction') as string) });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {}): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'PayOpenAccessEditView' + (this.$t('app.formpage.notconfig.loaddraftaction') as string) });
            return;
        }
        const arg: any = { ...opt } ;
        let viewparamResult:any = Object.assign(arg,this.viewparams);
    ...
    public autoSave(opt: any = {}): void {
        ...
        if(!action){
            ...
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'PayOpenAccessEditView' + (this.$t('app.formpage.notconfig.actionname') as string) });
            return;
        }
        ...
        post.then((response: any) => {
            ...
            this.$emit('save', data);
            AppCenterService.notifyMessage({name:"PayOpenAccess",action:'appRefresh',data:data});
            this.$nextTick(() => {
    ...
    */
    public remove(opt:Array<any> = [],showResultInfo?: boolean): Promise<any> {
        return new Promise((resolve: any, reject: any) => {
            if(!this.removeAction){
                this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'PayOpenAccessEditView' + (this.$t('app.formpage.notconfig.removeaction') as string) });
                return;
            }
            const arg: any = opt[0];
            const _this: any = this;
    ...
    public async wfsubmit(data: any,localdata?:any): Promise<any> {
        ...
        Object.assign(arg,{viewparams:this.viewparams});
        if (!arg.payopenaccess || Object.is(arg.payopenaccess, '')) {
            return;
        }

...
, 
        n_sys_rolename_like: new FormItemModel({ caption: '角色名称(%)', detailType: 'FORMITEM', name: 'n_sys_rolename_like', visible: true, isShowCaption: true, form: this,required:false, disabled: false, enableCond: 3 })
, 
    };
    ...
    public rules: any = {
        n_sys_rolename_like: [
            ...
            { required: this.detailsModel.n_sys_rolename_like.required, type: 'string', message: '角色名称(%) 值不能为空', trigger: 'change' },
            { required: this.detailsModel.n_sys_rolename_like.required, type: 'string', message: '角色名称(%) 值不能为空', trigger: 'blur' },
        ],
    }

    /**
     * 监控表单属性 n_sys_rolename_like 值
     *
     * @param {*} newVal
     * @param {*} oldVal
     * @memberof DefaultBase
     */
    @Watch('data.n_sys_rolename_like')
    onN_sys_rolename_likeChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'n_sys_rolename_like', newVal: newVal, oldVal: oldVal });
    }
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'SysRolePickupGridView_norepeat' + (this.$t('app.searchForm.notConfig.loadAction') as string) });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {},mode?:string): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'SysRolePickupGridView_norepeat' + (this.$t('app.searchForm.notConfig.loaddraftAction') as string) });
            return;
        }
        const arg: any = { ...opt } ;
        Object.assign(arg,{viewparams:this.viewparams});

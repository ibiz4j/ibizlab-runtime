  ...
  <i-form style="height:100%;display:flex;flex-direction: column;justify-content: space-between">
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            <template slot="empty">
                {{$t('entities.sysauthlog.main_grid.nodata')}} 
            </template>
            ...
            <template v-if="getColumnState('authtime')">
                <el-table-column show-overflow-tooltip :prop="'authtime'" :label="$t('entities.sysauthlog.main_grid.columns.authtime')" :width="250"  :align="'left'" :sortable="'custom'">
                    ...
                    <template v-slot="{row,column,$index}">
                        <app-format-data format="YYYY-MM-DD HH:mm:ss" :data="row.authtime"></app-format-data>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('authcode')">
                <el-table-column show-overflow-tooltip :prop="'authcode'" :label="$t('entities.sysauthlog.main_grid.columns.authcode')" :width="100"  :align="'left'" :sortable="'custom'">
                    ...
                    <template v-slot="{row,column,$index}">
                        <template >
            <codelist :value="row.authcode" tag='CLAuthCode' codelistType='STATIC' ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            ...
            <template v-if="getColumnState('macaddr')">
            ...
            <template v-if="getColumnState('useragent')">
...
import SysAuthLogEntityService from '@/service/sys-auth-log/sys-auth-log-service';
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof MainBase
     */
    public appEntityService: SysAuthLogEntityService = new SysAuthLogEntityService({ $store: this.$store });
    ...
    */
    ...
    public rules(){
        return {
            srfkey: [
                { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: `${this.$t('entities.sysauthlog.main_grid.columns.srfkey')}${this.$t('app.commonWords.valueNotEmpty')}`, trigger: 'change' },
                { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: `${this.$t('entities.sysauthlog.main_grid.columns.srfkey')}${this.$t('app.commonWords.valueNotEmpty')}`, trigger: 'blur' },
            ],
        }
    }
    ...
    public async remove(datas: any[]): Promise<any> {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ sysauthlog: _keys }),Object.assign({ sysauthlog: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    */
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.sysauthlog,$event.sysauthlog);
        });
    ...
    public setColState() {
        ...
        Object.assign(viewParams,{utilServiceName: 'grid_dynaconfig', modelid: 'ibzuaa_web_sysauthloggridview_grid_main'});
    ...
    public getColStorage() {
        const _data: any = localStorage.getItem('sys_authlog_main_grid');
        if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('sys_authlog_main_grid', JSON.stringify(this.allColumns));
    }
    ...
    public saveDynaConfig() {
        const viewParams: any = Util.deepCopy(this.viewparams);
        Object.assign(viewParams,{utilServiceName: 'grid_dynaconfig', modelid: 'ibzuaa_web_sysauthloggridview_grid_main', model: this.allColumns});
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    ...
                    if(!this.updateAction){
                        ...
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.sysauthlog){
                            Object.assign(this.context,{sysauthlog:item.sysauthlog});
                        }
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.sysauthlog,args.row.sysauthlog);
        });

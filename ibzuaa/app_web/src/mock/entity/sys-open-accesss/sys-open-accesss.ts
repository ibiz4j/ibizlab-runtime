...
//getwflink
mock.onGet(new RegExp(/^\/wfcore\/ibzuaa-app-web\/sysopenaccesses\/[a-zA-Z0-9\-\;]+\/usertasks\/[a-zA-Z0-9\-\;]+\/ways$/)).reply((config: any) => {
    console.groupCollapsed("实体:sysopenaccess 方法: getwflink");
...
// getwfstep
mock.onGet(new RegExp(/^\/wfcore\/ibzuaa-app-web\/sysopenaccesses\/process-definitions-nodes$/)).reply((config: any) => {
    console.groupCollapsed("实体:sysopenaccess 方法: getwfstep");
    console.table({url:config.url, method: config.method, data:config.data});
    console.groupEnd();
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
...
// createBatch
mock.onPost(new RegExp(/^\/sysopenaccesses\/batch$/)).reply((config: any) => {
    console.groupCollapsed("实体:sysopenaccess 方法: createBatch");
    console.table({url:config.url, method: config.method, data:config.data});
    console.groupEnd();
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
...
// updateBatch
mock.onPut(new RegExp(/^\/sysopenaccesses\/batch$/)).reply((config: any) => {
    console.groupCollapsed("实体:sysopenaccess 方法: updateBatch");
    console.table({url:config.url, method: config.method, data:config.data});
    console.groupEnd();
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
...
// removeBatch
mock.onDelete(new RegExp(/^\/sysopenaccesses\/batch$/)).reply((config: any) => {
    console.groupCollapsed("实体:sysopenaccess 方法: removeBatch");
    console.table({url:config.url, method: config.method, data:config.data});
    console.groupEnd();
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
...
// Create
mock.onPost(new RegExp(/^\/sysopenaccesses\/?([a-zA-Z0-9\-\;]{0,35})$/)).reply((config: any) => {
    console.groupCollapsed("实体:sysopenaccess 方法: Create");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
        return [status, null];
...
// FetchDefault
mock.onGet(new RegExp(/^\/sysopenaccesses\/fetchdefault$/)).reply((config: any) => {
    console.groupCollapsed("实体:sysopenaccess 方法: FetchDefault");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
        return [status, null];
...
// FetchDefault
mock.onGet(new RegExp(/^\/sysopenaccesses\/fetchdefault(\?[\w-./?%&=,]*)*$/)).reply((config: any) => {
    console.groupCollapsed("实体:sysopenaccess 方法: FetchDefault");
    console.table({url:config.url, method: config.method, data:config.data});

...
//getwflink
mock.onGet(new RegExp(/^\/wfcore\/ibzrt-app-web\/ibzteams\/[a-zA-Z0-9\-\;]+\/usertasks\/[a-zA-Z0-9\-\;]+\/ways$/)).reply((config: any) => {
...
// getwfstep
mock.onGet(new RegExp(/^\/wfcore\/ibzrt-app-web\/ibzteams\/process-definitions-nodes$/)).reply((config: any) => {
...
// Save
mock.onPost(new RegExp(/^\/ibzteams\/?([a-zA-Z0-9\-\;]{0,35})\/save$/)).reply((config: any) => {
    console.groupCollapsed("实体:ibzteam 方法: Save");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
        return [status, null];
    ...
    const paramArray:Array<any> = ['teamid'];
    const matchArray:any = new RegExp(/^\/ibzteams\/([a-zA-Z0-9\-\;]{1,35})\/save$/).exec(config.url);
    let tempValue: any = {};
    if(matchArray && matchArray.length >1 && paramArray && paramArray.length >0){
        paramArray.forEach((item: any, index: number) => {
            Object.defineProperty(tempValue, item, {
...
// FetchDefault
mock.onGet(new RegExp(/^\/ibzteams\/fetchdefault$/)).reply((config: any) => {
...
// FetchDefault
mock.onGet(new RegExp(/^\/ibzteams\/fetchdefault(\?[\w-./?%&=,]*)*$/)).reply((config: any) => {
    console.groupCollapsed("实体:ibzteam 方法: FetchDefault");
    console.table({url:config.url, method: config.method, data:config.data});
...
// Remove
mock.onDelete(new RegExp(/^\/ibzteams\/([a-zA-Z0-9\-\;]{1,35})$/)).reply((config: any) => {
    console.groupCollapsed("实体:ibzteam 方法: Remove");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
        return [status, null];
...
// Get
mock.onGet(new RegExp(/^\/ibzteams\/([a-zA-Z0-9\-\;]{1,35})$/)).reply((config: any) => {
    console.groupCollapsed("实体:ibzteam 方法: Get");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
        return [status, null];

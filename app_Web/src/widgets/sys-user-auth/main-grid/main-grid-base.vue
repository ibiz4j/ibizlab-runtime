  ...
  <i-form style="height:100%;display:flex;flex-direction: column;justify-content: space-between">
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            <template slot="empty">
                {{$t('entities.sysuserauth.main_grid.nodata')}} 
            </template>
            <template v-if="!isSingleSelect">
                <el-table-column align="center" type='selection' :width="checkboxColWidth"></el-table-column>
            </template>
            <template v-if="getColumnState('username')">
                <el-table-column show-overflow-tooltip :prop="'username'" :label="$t('entities.sysuserauth.main_grid.columns.username')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.sysuserauth.main_grid.columns.username')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('identity_type')">
                <el-table-column show-overflow-tooltip :prop="'identity_type'" :label="$t('entities.sysuserauth.main_grid.columns.identity_type')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.sysuserauth.main_grid.columns.identity_type')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('identifier')">
                <el-table-column show-overflow-tooltip :prop="'identifier'" :label="$t('entities.sysuserauth.main_grid.columns.identifier')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.sysuserauth.main_grid.columns.identifier')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('credential')">
                <el-table-column show-overflow-tooltip :prop="'credential'" :label="$t('entities.sysuserauth.main_grid.columns.credential')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.sysuserauth.main_grid.columns.credential')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
...
import SysUserAuthEntityService from '@/service/sys-user-auth/sys-user-auth-service';
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof MainBase
     */
    public appEntityService: SysUserAuthEntityService = new SysUserAuthEntityService({ $store: this.$store });
     ...
     * @memberof MainBase
     */  
    public majorInfoColName:string = "username";
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'username',
            label: '人员',
            langtag: 'entities.sysuserauth.main_grid.columns.username',
            show: true,
            unit: 'PX',
        ...
        },
        {
            name: 'identity_type',
            label: '认证类型',
            langtag: 'entities.sysuserauth.main_grid.columns.identity_type',
            show: true,
            unit: 'PX',
            isEnableRowEdit: false,
            enableCond: 3 ,
        },
        {
            name: 'identifier',
            label: '认证标识',
            langtag: 'entities.sysuserauth.main_grid.columns.identifier',
            show: true,
            unit: 'PX',
            isEnableRowEdit: false,
            enableCond: 3 ,
        },
        {
            name: 'credential',
            label: '凭据',
            langtag: 'entities.sysuserauth.main_grid.columns.credential',
            show: true,
            unit: 'PX',
            isEnableRowEdit: false,
            enableCond: 3 ,
    ...
    public rules(){
        return {
            srfkey: [
                { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: `${this.$t('entities.sysuserauth.main_grid.columns.srfkey')}${this.$t('app.commonWords.valueNotEmpty')}`, trigger: 'change' },
                { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: `${this.$t('entities.sysuserauth.main_grid.columns.srfkey')}${this.$t('app.commonWords.valueNotEmpty')}`, trigger: 'blur' },
            ],
        }
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'SysUserAuthGridView'+(this.$t('app.gridpage.notConfig.fetchAction') as string) });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'SysUserAuthGridView'+(this.$t('app.gridpage.notConfig.removeAction') as string) });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.username;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ sysuserauth: _keys }),Object.assign({ sysuserauth: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
        ...
        const doExport = async (_data:any) => {
            ...
            this.$export.exportExcel().then((excel:any)=>{
                excel.export_json_to_excel({
                  ...
                  data, //具体数据 必填
                  filename: "账号绑定"+(this.$t('app.gridpage.grid') as string), //非必填
                  autoWidth: true, //非必填
                  bookType: "xlsx" //非必填
                });
            }); 
    ...
    */
    public arraySpanMethod({row, column, rowIndex, columnIndex} : any) {
        let allColumns:Array<any> = ['username','identity_type','identifier','credential'];
        if(row && row.children) {
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.sysuserauth,$event.sysuserauth);
        });
    ...
    public setColState() {
        ...
        Object.assign(viewParams,{utilServiceName: 'grid_dynaconfig', modelid: 'ibzrt_web_sysuserauthgridview_grid_main'});
    ...
    public getColStorage() {
        const _data: any = localStorage.getItem('sys_user_auth_main_grid');
        if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('sys_user_auth_main_grid', JSON.stringify(this.allColumns));
    }
    ...
    public saveDynaConfig() {
        const viewParams: any = Util.deepCopy(this.viewparams);
        Object.assign(viewParams,{utilServiceName: 'grid_dynaconfig', modelid: 'ibzrt_web_sysuserauthgridview_grid_main', model: this.allColumns});
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'SysUserAuthGridView'+(this.$t('app.gridpage.notConfig.createAction') as string) });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'SysUserAuthGridView'+(this.$t('app.gridpage.notConfig.updateAction') as string) });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.sysuserauth){
                            Object.assign(this.context,{sysuserauth:item.sysuserauth});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'SysUserAuthGridView'+(this.$t('app.gridpage.notConfig.loaddraftAction') as string) });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.sysuserauth,args.row.sysuserauth);
        });

...
export default class MainBase extends Vue implements ControlInterface {
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'accessname',
            label: '开放平台',
            langtag: 'entities.payopenaccess.main_grid.columns.accessname',
            show: true,
            unit: 'px',
        ...
        },
        {
            name: 'open_type',
            label: '开放平台类型',
            langtag: 'entities.payopenaccess.main_grid.columns.open_type',
            show: true,
            unit: 'px',
            isEnableRowEdit: false,
            enableCond: 3 ,
        },
        {
            name: 'region_id',
            label: 'RegionId',
            langtag: 'entities.payopenaccess.main_grid.columns.region_id',
            show: true,
            unit: 'px',
            isEnableRowEdit: false,
            enableCond: 3 ,
        },
        {
            name: 'redirect_uri',
            label: 'RedirectURI',
            langtag: 'entities.payopenaccess.main_grid.columns.redirect_uri',
            show: true,
            unit: 'px',
            isEnableRowEdit: false,
            enableCond: 3 ,
    ...
    public rules(){
        return {
            srfkey: [
                { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: `${this.$t('entities.payopenaccess.main_grid.columns.srfkey')}${this.$t('app.commonWords.valueNotEmpty')}`, trigger: 'change' },
                { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: `${this.$t('entities.payopenaccess.main_grid.columns.srfkey')}${this.$t('app.commonWords.valueNotEmpty')}`, trigger: 'blur' },
            ],
        }
    }
    ...
    public async remove(datas: any[]): Promise<any> {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ payopenaccess: _keys }),Object.assign({ payopenaccess: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
        ...
        const doExport = async (_data:any) => {
            ...
            this.$export.exportExcel().then((excel:any)=>{
                excel.export_json_to_excel({
                  ...
                  data, //具体数据 必填
                  filename: "支付平台"+(this.$t('app.gridpage.grid') as string), //非必填
                  autoWidth: true, //非必填
                  bookType: "xlsx" //非必填
                });
            }); 
    ...
    public afterCreated(){
        ...
        if(AppCenterService && AppCenterService.getMessageCenter()){
            this.appStateEvent = AppCenterService.getMessageCenter().subscribe(({ name, action, data }) =>{
                if(!Object.is(name,"PayOpenAccess")){
                    return;
                }
    ...
    */
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.payopenaccess,$event.payopenaccess);
        });
    ...
    public setColState() {
        ...
        Object.assign(viewParams,{utilServiceName: 'grid_dynaconfig', modelid: 'ibzrt_web_payopenaccesspickupgridview_grid_main'});
    ...
    public getColStorage() {
        const _data: any = localStorage.getItem('pay_open_access_main_grid');
        if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('pay_open_access_main_grid', JSON.stringify(this.allColumns));
    }
    ...
    public saveDynaConfig() {
        const viewParams: any = Util.deepCopy(this.viewparams);
        Object.assign(viewParams,{utilServiceName: 'grid_dynaconfig', modelid: 'ibzrt_web_payopenaccesspickupgridview_grid_main', model: this.allColumns});
                ...
                if(Object.is(item.rowDataState, 'create')){
                    ...
                    if(!this.updateAction){
                        ...
                        if(item.payopenaccess){
                            Object.assign(this.context,{payopenaccess:item.payopenaccess});
                        }
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.payopenaccess,args.row.payopenaccess);
        });

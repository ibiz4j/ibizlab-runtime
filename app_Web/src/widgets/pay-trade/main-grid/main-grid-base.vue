...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof MainBase
     */  
    public majorInfoColName:string = "tradename";
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'tradename',
            label: '交易名称',
            langtag: 'entities.paytrade.main_grid.columns.tradename',
            show: true,
            unit: 'px',
        ...
        },
        {
            name: 'subject',
            label: '订单标题',
            langtag: 'entities.paytrade.main_grid.columns.subject',
            show: true,
            unit: 'px',
            isEnableRowEdit: false,
            enableCond: 3 ,
        },
        {
            name: 'outtradeno',
            label: '订单号',
            langtag: 'entities.paytrade.main_grid.columns.outtradeno',
            show: true,
            unit: 'px',
            isEnableRowEdit: false,
            enableCond: 3 ,
        },
        {
            name: 'totalamount',
            label: '订单金额',
            langtag: 'entities.paytrade.main_grid.columns.totalamount',
            show: true,
            unit: 'px',
            isEnableRowEdit: false,
            enableCond: 3 ,
        },
        {
            name: 'accessname',
            label: '支付平台',
            langtag: 'entities.paytrade.main_grid.columns.accessname',
            show: true,
            unit: 'px',
            isEnableRowEdit: false,
            enableCond: 3 ,
    ...
    public rules(){
        return {
            srfkey: [
                { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: `${this.$t('entities.paytrade.main_grid.columns.srfkey')}${this.$t('app.commonWords.valueNotEmpty')}`, trigger: 'change' },
                { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: `${this.$t('entities.paytrade.main_grid.columns.srfkey')}${this.$t('app.commonWords.valueNotEmpty')}`, trigger: 'blur' },
            ],
        }
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'PayTradeGridView'+(this.$t('app.gridpage.notConfig.fetchAction') as string) });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'PayTradeGridView'+(this.$t('app.gridpage.notConfig.removeAction') as string) });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.tradename;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ paytrade: _keys }),Object.assign({ paytrade: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    */
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.paytrade,$event.paytrade);
        });
    ...
    public setColState() {
        ...
        Object.assign(viewParams,{utilServiceName: 'grid_dynaconfig', modelid: 'ibzrt_web_paytradegridview_grid_main'});
    ...
    public getColStorage() {
        const _data: any = localStorage.getItem('pay_trade_main_grid');
        if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('pay_trade_main_grid', JSON.stringify(this.allColumns));
    }
    ...
    public saveDynaConfig() {
        const viewParams: any = Util.deepCopy(this.viewparams);
        Object.assign(viewParams,{utilServiceName: 'grid_dynaconfig', modelid: 'ibzrt_web_paytradegridview_grid_main', model: this.allColumns});
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'PayTradeGridView'+(this.$t('app.gridpage.notConfig.createAction') as string) });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'PayTradeGridView'+(this.$t('app.gridpage.notConfig.updateAction') as string) });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.paytrade){
                            Object.assign(this.context,{paytrade:item.paytrade});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'PayTradeGridView'+(this.$t('app.gridpage.notConfig.loaddraftAction') as string) });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.paytrade,args.row.paytrade);
        });
        ...
        if(Object.is(action,"UPDATE")){
            ...
            if(nativeData && (nativeData instanceof Array) && nativeData.length >0){
                let targetData:any = nativeData.find((item:any) =>{
                    return item.trade_id === data.srfkey;
                })
                if(targetData){

...
, 
        n_sys_username_like: new FormItemModel({ caption: '用户名称(%)', detailType: 'FORMITEM', name: 'n_sys_username_like', visible: true, isShowCaption: true, form: this,required:false, disabled: false, enableCond: 3 })
, 
    };
    ...
    public rules: any = {
        n_sys_username_like: [
            ...
            { required: this.detailsModel.n_sys_username_like.required, type: 'string', message: '用户名称(%) 值不能为空', trigger: 'change' },
            { required: this.detailsModel.n_sys_username_like.required, type: 'string', message: '用户名称(%) 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'SysUserRoleGridView' + (this.$t('app.searchForm.notConfig.loadAction') as string) });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {},mode?:string): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'SysUserRoleGridView' + (this.$t('app.searchForm.notConfig.loaddraftAction') as string) });
            return;
        }
        const arg: any = { ...opt } ;
        Object.assign(arg,{viewparams:this.viewparams});

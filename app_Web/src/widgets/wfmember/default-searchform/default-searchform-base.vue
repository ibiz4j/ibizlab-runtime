...
export default class DefaultBase extends Vue implements ControlInterface {
    ...
    public rules: any = {
        n_personname_like: [
            ...
            { required: false, type: 'string', message: '用户(文本包含(%)) 值不能为空', trigger: 'change' },
            { required: false, type: 'string', message: '用户(文本包含(%)) 值不能为空', trigger: 'blur' },
        ],
    }
...
, 
        n_personname_like: new FormItemModel({ caption: '用户(文本包含(%))', detailType: 'FORMITEM', name: 'n_personname_like', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
    };
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'WFMemberGridView视图搜索表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {},mode?:string): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'WFMemberGridView视图搜索表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        Object.assign(arg,{viewparams:this.viewparams});

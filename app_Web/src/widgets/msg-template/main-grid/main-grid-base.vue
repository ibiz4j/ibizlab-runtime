  ...
  <i-form style="height:100%;display:flex;flex-direction: column;justify-content: space-between">
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            <template slot="empty">
                {{$t('entities.msgtemplate.main_grid.nodata')}} 
            </template>
            <template v-if="!isSingleSelect">
                <el-table-column align="center" type='selection' :width="checkboxColWidth"></el-table-column>
            </template>
            <template v-if="getColumnState('template_type')">
                <el-table-column show-overflow-tooltip :prop="'template_type'" :label="$t('entities.msgtemplate.main_grid.columns.template_type')" :width="200"  :align="'left'" :sortable="'custom'">
                    ...
                    <template v-slot="{row,column,$index}">
                        <template >
            <codelist :value="row.template_type" tag='TemplateType' codelistType='STATIC' ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('template_id')">
                <el-table-column show-overflow-tooltip :prop="'template_id'" :label="$t('entities.msgtemplate.main_grid.columns.template_id')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
            ...
            <template v-if="getColumnState('template_name')">
                <el-table-column show-overflow-tooltip :prop="'template_name'" :label="$t('entities.msgtemplate.main_grid.columns.template_name')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
            ...
            <template v-if="getColumnState('open_type')">
                <el-table-column show-overflow-tooltip :prop="'open_type'" :label="$t('entities.msgtemplate.main_grid.columns.open_type')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
            ...
            <template v-if="getColumnState('accessname')">
                <el-table-column show-overflow-tooltip :prop="'accessname'" :label="$t('entities.msgtemplate.main_grid.columns.accessname')" :width="300"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
...
export default class MainBase extends Vue implements ControlInterface {
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'template_type',
            label: '模板类型',
            langtag: 'entities.msgtemplate.main_grid.columns.template_type',
            show: true,
            unit: 'PX',
        ...
        },
        {
            name: 'template_id',
            label: '模板标识',
            langtag: 'entities.msgtemplate.main_grid.columns.template_id',
            show: true,
            unit: 'PX',
            isEnableRowEdit: false,
            enableCond: 3 ,
        },
        {
            name: 'template_name',
            label: '模板标题',
            langtag: 'entities.msgtemplate.main_grid.columns.template_name',
            show: true,
            unit: 'PX',
            isEnableRowEdit: false,
            enableCond: 3 ,
        },
        {
            name: 'open_type',
            label: '接入平台类型',
            langtag: 'entities.msgtemplate.main_grid.columns.open_type',
            show: true,
            unit: 'PX',
            isEnableRowEdit: false,
            enableCond: 3 ,
        },
        {
            name: 'accessname',
            label: '接入平台',
            langtag: 'entities.msgtemplate.main_grid.columns.accessname',
            show: true,
            unit: 'PX',
            isEnableRowEdit: false,
            enableCond: 3 ,
    ...
    public rules(){
        return {
            srfkey: [
                { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: `${this.$t('entities.msgtemplate.main_grid.columns.srfkey')}${this.$t('app.commonWords.valueNotEmpty')}`, trigger: 'change' },
                { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: `${this.$t('entities.msgtemplate.main_grid.columns.srfkey')}${this.$t('app.commonWords.valueNotEmpty')}`, trigger: 'blur' },
            ],
        }
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'MsgTemplateGridView'+(this.$t('app.gridpage.notConfig.fetchAction') as string) });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'MsgTemplateGridView'+(this.$t('app.gridpage.notConfig.removeAction') as string) });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.template_name;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ msgtemplate: _keys }),Object.assign({ msgtemplate: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
        ...
        const doExport = async (_data:any) => {
            ...
            this.$export.exportExcel().then((excel:any)=>{
                excel.export_json_to_excel({
                  ...
                  data, //具体数据 必填
                  filename: "消息模板"+(this.$t('app.gridpage.grid') as string), //非必填
                  autoWidth: true, //非必填
                  bookType: "xlsx" //非必填
                });
            }); 
    ...
    */
    public arraySpanMethod({row, column, rowIndex, columnIndex} : any) {
        let allColumns:Array<any> = ['template_type','template_id','template_name','open_type','accessname'];
        if(row && row.children) {
    ...
    public setColState() {
        ...
        Object.assign(viewParams,{utilServiceName: 'grid_dynaconfig', modelid: 'ibzrt_web_msgtemplategridview_grid_main'});
    ...
    public getColStorage() {
        const _data: any = localStorage.getItem('msg_template_main_grid');
        if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('msg_template_main_grid', JSON.stringify(this.allColumns));
    }
    ...
    public saveDynaConfig() {
        const viewParams: any = Util.deepCopy(this.viewparams);
        Object.assign(viewParams,{utilServiceName: 'grid_dynaconfig', modelid: 'ibzrt_web_msgtemplategridview_grid_main', model: this.allColumns});
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'MsgTemplateGridView'+(this.$t('app.gridpage.notConfig.createAction') as string) });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'MsgTemplateGridView'+(this.$t('app.gridpage.notConfig.updateAction') as string) });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.msgtemplate){
                            Object.assign(this.context,{msgtemplate:item.msgtemplate});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'MsgTemplateGridView'+(this.$t('app.gridpage.notConfig.loaddraftAction') as string) });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.msgtemplate,args.row.msgtemplate);
        });

  ...
  <row>
    <i-col span="20" class="form-content">
      <row>
              ...
              <app-form-item name='n_accessname_like' :itemRules="this.rules.n_accessname_like" class='' :caption="$t('entities.msgtemplate.default_searchform.details.n_accessname_like')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.n_accessname_like.error" :isEmptyCaption="false" labelPos="LEFT"> 
              <input-box 
          ...
          <i-col v-show="detailsModel.n_template_name_like.visible" :style="{}"  :sm="{ span: 24, offset: 0 }" :md="{ span: 12, offset: 0 }" :lg="{ span: 12, offset: 0 }" :xl="{ span: 12, offset: 0 }">
              <app-form-item name='n_template_name_like' :itemRules="this.rules.n_template_name_like" class='' :caption="$t('entities.msgtemplate.default_searchform.details.n_template_name_like')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.n_template_name_like.error" :isEmptyCaption="false" labelPos="LEFT"> 
              <input-box 
              v-model="data.n_template_name_like"  
              @enter="onEnter($event)"  
                
              :disabled="detailsModel.n_template_name_like.disabled" 
              type='text' 
              style="">
          </input-box>
...
, 
    ...
    /**
     * 监控表单属性 n_template_name_like 值
     *
     * @param {*} newVal
     * @param {*} oldVal
     * @memberof DefaultBase
     */
    @Watch('data.n_template_name_like')
    onN_template_name_likeChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'n_template_name_like', newVal: newVal, oldVal: oldVal });
    }
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'MsgTemplateGridView' + (this.$t('app.searchForm.notConfig.loadAction') as string) });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {},mode?:string): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'MsgTemplateGridView' + (this.$t('app.searchForm.notConfig.loaddraftAction') as string) });
            return;
        }
        const arg: any = { ...opt } ;
        Object.assign(arg,{viewparams:this.viewparams});

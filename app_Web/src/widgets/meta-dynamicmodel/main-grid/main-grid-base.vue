  ...
  <i-form style="height:100%;display:flex;flex-direction: column;justify-content: space-between">
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            <template slot="empty">
                {{$t('entities.meta_dynamicmodel.main_grid.nodata')}} 
            </template>
            <template v-if="!isSingleSelect">
                <el-table-column align="center" type='selection' :width="checkboxColWidth"></el-table-column>
            </template>
            <template v-if="getColumnState('configname')">
                <el-table-column show-overflow-tooltip :prop="'configname'" :label="$t('entities.meta_dynamicmodel.main_grid.columns.configname')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.meta_dynamicmodel.main_grid.columns.configname')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('systemid')">
                <el-table-column show-overflow-tooltip :prop="'systemid'" :label="$t('entities.meta_dynamicmodel.main_grid.columns.systemid')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.meta_dynamicmodel.main_grid.columns.systemid')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('dynainstid')">
                <el-table-column show-overflow-tooltip :prop="'dynainstid'" :label="$t('entities.meta_dynamicmodel.main_grid.columns.dynainstid')" :width="350"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.meta_dynamicmodel.main_grid.columns.dynainstid')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('status')">
                <el-table-column show-overflow-tooltip :prop="'status'" :label="$t('entities.meta_dynamicmodel.main_grid.columns.status')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.meta_dynamicmodel.main_grid.columns.status')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
...
import META_DYNAMICMODELEntityService from '@/service/meta-dynamicmodel/meta-dynamicmodel-service';
import MainService from './main-grid-service';
import META_DYNAMICMODELUIService from '@/uiservice/meta-dynamicmodel/meta-dynamicmodel-ui-service';
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * 实体服务对象
     *
     * @type {META_DYNAMICMODELService}
     * @memberof MainBase
     */
    public appEntityService: META_DYNAMICMODELEntityService = new META_DYNAMICMODELEntityService({ $store: this.$store });
     ...
     * 界面UI服务对象
     *
     * @type {META_DYNAMICMODELUIService}
     * @memberof MainBase
     */  
    public appUIService:META_DYNAMICMODELUIService = new META_DYNAMICMODELUIService(this.$store);
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'configname',
            label: '配置名称',
            langtag: 'entities.meta_dynamicmodel.main_grid.columns.configname',
            show: true,
            unit: 'PX',
        ...
        },
        {
            name: 'systemid',
            label: '系统',
            langtag: 'entities.meta_dynamicmodel.main_grid.columns.systemid',
            show: true,
            unit: 'PX',
            isEnableRowEdit: false,
            enableCond: 3 ,
        },
        {
            name: 'dynainstid',
            label: '动态实例',
            langtag: 'entities.meta_dynamicmodel.main_grid.columns.dynainstid',
            show: true,
            unit: 'PX',
            isEnableRowEdit: false,
            enableCond: 3 ,
        },
        {
            name: 'status',
            label: '状态',
            langtag: 'entities.meta_dynamicmodel.main_grid.columns.status',
            show: true,
            unit: 'PX',
            isEnableRowEdit: false,
            enableCond: 3 ,
    ...
    public rules(){
        return {
            srfkey: [
                { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: `${this.$t('entities.meta_dynamicmodel.main_grid.columns.srfkey')}${this.$t('app.commonWords.valueNotEmpty')}`, trigger: 'change' },
                { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: `${this.$t('entities.meta_dynamicmodel.main_grid.columns.srfkey')}${this.$t('app.commonWords.valueNotEmpty')}`, trigger: 'blur' },
            ],
        }
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'META_DYNAMICMODELDynaInstGridView'+(this.$t('app.gridpage.notConfig.fetchAction') as string) });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'META_DYNAMICMODELDynaInstGridView'+(this.$t('app.gridpage.notConfig.removeAction') as string) });
            return;
        }
        let _datas:any[] = [];
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ meta_dynamicmodel: _keys }),Object.assign({ meta_dynamicmodel: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public afterCreated(){
        ...
        if(AppCenterService && AppCenterService.getMessageCenter()){
            this.appStateEvent = AppCenterService.getMessageCenter().subscribe(({ name, action, data }) =>{
                if(!Object.is(name,"META_DYNAMICMODEL")){
                    return;
                }
    ...
    */
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.meta_dynamicmodel,$event.meta_dynamicmodel);
        });
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'META_DYNAMICMODELDynaInstGridView'+(this.$t('app.gridpage.notConfig.createAction') as string) });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'META_DYNAMICMODELDynaInstGridView'+(this.$t('app.gridpage.notConfig.updateAction') as string) });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.meta_dynamicmodel){
                            Object.assign(this.context,{meta_dynamicmodel:item.meta_dynamicmodel});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'META_DYNAMICMODELDynaInstGridView'+(this.$t('app.gridpage.notConfig.loaddraftAction') as string) });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.meta_dynamicmodel,args.row.meta_dynamicmodel);
        });

<template>
    <i-form :model="this.data" class='app-form' ref='form'  id='meta_dynamicmodel_dynainst' style="" @on-validate="formItemValidate">
    <input style="display:none;" />
    <row >
            <i-col v-show="detailsModel.group1.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-group :uiService="appUIService" :data="transformData(data)" :manageContainerStatus="detailsModel.group1.manageContainerStatus"  :isManageContainer="detailsModel.group1.isManageContainer" @managecontainerclick="manageContainerClick('group1')" layoutType="TABLE_24COL" titleStyle="" class='' :uiActionGroup="detailsModel.group1.uiActionGroup" @groupuiactionclick="groupUIActionClick($event)" :caption="$t('entities.meta_dynamicmodel.dynainst_form.details.group1')" :isShowCaption="true" uiStyle="DEFAULT" :titleBarCloseMode="0" :isInfoGroupMode="false" >    
    <row>
        <i-col v-show="detailsModel.configname.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-item name='configname' :itemRules="this.rules().configname" class='' :caption="$t('entities.meta_dynamicmodel.dynainst_form.details.configname')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.configname.error" :isEmptyCaption="false" labelPos="LEFT">
    <input-box 
...
<i-col v-show="detailsModel.formitem.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-item name='formitem' :itemRules="this.rules().formitem" class='' :caption="$t('entities.meta_dynamicmodel.dynainst_form.details.formitem')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.formitem.error" :isEmptyCaption="false" labelPos="LEFT">
...
<i-col v-show="detailsModel.tabpanel1.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
        ...
        <tab-pane v-show="detailsModel.tabpage1.visible" name='tabpage1' :index="0" tab='tabpanel1' class=''  
            :label="(h) =>{
                return h('span',{
                  ...
                  $t('entities.meta_dynamicmodel.dynainst_form.details.tabpage1')
                ])
            }">
    ...
    :localParam ='{}'
    parameterName='meta_dynamicmodel'
    parentName="META_DYNAMICMODEL"  
    refviewtype='DEGRIDVIEW' 
    refreshitems='' 
    :ignorefieldvaluechange="ignorefieldvaluechange"
    viewname='meta-dynamicmodelgrid-view'
    tempMode='0'
...
, 
        group1: new FormGroupPanelModel({ caption: '动态模型基本信息', detailType: 'GROUPPANEL', name: 'group1', visible: true, isShowCaption: true, form: this, isControlledContent: false , uiActionGroup: { caption: '', langbase: 'entities.meta_dynamicmodel.dynainst_form', extractMode: 'ITEM', details: [] }, isManageContainer: false, showMoreModeItems: []  })
, 
...
, 
    ...
    public onFormLoad(data: any = {},action:string): void {
        ...
        // 更新context的实体主键
        if(data.meta_dynamicmodel){
            Object.assign(this.context,{meta_dynamicmodel:data.meta_dynamicmodel})
        }
    ...
    public afterCreated(){
        if(this.isautoload){
            this.autoLoad({srfkey:this.context.meta_dynamicmodel});
        }
        if (this.viewState) {
    ...
    public print(){
        let _this:any = this;
        _this.$print({id:'meta_dynamicmodel_dynainst',popTitle:'主实例及副本实例关系表单'});
    }
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'META_DYNAMICMODELDynaInstEditView' + (this.$t('app.formpage.notconfig.loadaction') as string) });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {}): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'META_DYNAMICMODELDynaInstEditView' + (this.$t('app.formpage.notconfig.loaddraftaction') as string) });
            return;
        }
        const arg: any = { ...opt } ;
        let viewparamResult:any = Object.assign(arg,this.viewparams);
    ...
    public autoSave(opt: any = {}): void {
        ...
        if(!action){
            ...
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'META_DYNAMICMODELDynaInstEditView' + (this.$t('app.formpage.notconfig.actionname') as string) });
            return;
        }
        ...
        post.then((response: any) => {
            ...
            this.$emit('save', data);
            AppCenterService.notifyMessage({name:"META_DYNAMICMODEL",action:'appRefresh',data:data});
            this.$nextTick(() => {
    ...
    */
    public remove(opt:Array<any> = [],showResultInfo?: boolean): Promise<any> {
        return new Promise((resolve: any, reject: any) => {
            if(!this.removeAction){
                this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'META_DYNAMICMODELDynaInstEditView' + (this.$t('app.formpage.notconfig.removeaction') as string) });
                return;
            }
            const arg: any = opt[0];
            const _this: any = this;
    ...
    public async wfsubmit(data: any,localdata?:any): Promise<any> {
        ...
        Object.assign(arg,{viewparams:this.viewparams});
        if (!arg.meta_dynamicmodel || Object.is(arg.meta_dynamicmodel, '')) {
            return;
        }

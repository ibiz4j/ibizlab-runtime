  ...
  <row>
    <i-col span="20" class="form-content">
      <row>
              ...
              <app-form-item name='orgcode' :itemRules="this.rules.orgcode" class='' :caption="$t('entities.sysorganization.default_searchform.details.orgcode')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.orgcode.error" :isEmptyCaption="false" labelPos="LEFT"> 
              <input-box 
          ...
          <i-col v-show="detailsModel.n_orgname_like.visible" :style="{}"  :sm="{ span: 12, offset: 0 }" :md="{ span: 12, offset: 0 }" :lg="{ span: 12, offset: 0 }" :xl="{ span: 8, offset: 0 }">
              <app-form-item name='n_orgname_like' :itemRules="this.rules.n_orgname_like" class='' :caption="$t('entities.sysorganization.default_searchform.details.n_orgname_like')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.n_orgname_like.error" :isEmptyCaption="false" labelPos="LEFT"> 
              <input-box 
          ...
          <i-col v-show="detailsModel.n_porgname_eq.visible" :style="{}"  :sm="{ span: 12, offset: 0 }" :md="{ span: 12, offset: 0 }" :lg="{ span: 12, offset: 0 }" :xl="{ span: 8, offset: 0 }">
              <app-form-item name='n_porgname_eq' :itemRules="this.rules.n_porgname_eq" class='' :caption="$t('entities.sysorganization.default_searchform.details.n_porgname_eq')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.n_porgname_eq.error" :isEmptyCaption="false" labelPos="LEFT"> 
              <app-org-select :data="data" :disabled="detailsModel.n_porgname_eq.disabled" :context="JSON.parse(JSON.stringify(context))" :fillMap="{'id':'n_porgid_eq','label':'n_porgname_eq'}" url="/ibzorganizations/alls/suborg/picker" filter="srforgid" :multiple="false" style="" @select-change="onFormItemValueChange" ></app-org-select>
          </app-form-item>
...
, 
        n_porgname_eq: new FormItemModel({ caption: '上级单位(等于(=))', detailType: 'FORMITEM', name: 'n_porgname_eq', visible: true, isShowCaption: true, form: this,required:false, disabled: false, enableCond: 3 })
, 
    };
    ...
    public rules: any = {
        ...
        n_porgname_eq: [
            ...
            { required: this.detailsModel.n_porgname_eq.required, type: 'string', message: '上级单位(等于(=)) 值不能为空', trigger: 'change' },
            { required: this.detailsModel.n_porgname_eq.required, type: 'string', message: '上级单位(等于(=)) 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    /**
     * 监控表单属性 n_porgid_eq 值
     *
     * @param {*} newVal
     * @param {*} oldVal
     * @memberof DefaultBase
     */
    @Watch('data.n_porgid_eq')
    onN_porgid_eqChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'n_porgid_eq', newVal: newVal, oldVal: oldVal });
    }
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'SysOrganizationGridView' + (this.$t('app.searchForm.notConfig.loadAction') as string) });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {},mode?:string): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'SysOrganizationGridView' + (this.$t('app.searchForm.notConfig.loaddraftAction') as string) });
            return;
        }
        const arg: any = { ...opt } ;
        Object.assign(arg,{viewparams:this.viewparams});

<template>
    <i-form :model="this.data" class='app-form' ref='form'  id='jobslog_main' style="">
...
, 
        trigger_code: new FormItemModel({ caption: '触发器调度返回码', detailType: 'FORMITEM', name: 'trigger_code', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
        trigger_type: new FormItemModel({ caption: '触发器调度类型', detailType: 'FORMITEM', name: 'trigger_type', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
        trigger_msg: new FormItemModel({ caption: '触发器调度信息', detailType: 'FORMITEM', name: 'trigger_msg', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
...
, 
    ...
    onJob_idChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'job_id', newVal: newVal, oldVal: oldVal });
    }
    ...
    onTrigger_codeChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'trigger_code', newVal: newVal, oldVal: oldVal });
    }

    /**
     * 监控表单属性 trigger_type 值
     *
     * @param {*} newVal
     * @param {*} oldVal
     * @memberof MainBase
    ...
    onTrigger_typeChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'trigger_type', newVal: newVal, oldVal: oldVal });
    }

    /**
     * 监控表单属性 trigger_msg 值
     *
     * @param {*} newVal
     * @param {*} oldVal
     * @memberof MainBase
    ...
    onTrigger_msgChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'trigger_msg', newVal: newVal, oldVal: oldVal });
    }
    ...
    public copy(srfkey: string): void {
        ...
        copyData.jobslog = copyData.srfkey;
        copyData.id = copyData.srfkey;
        Object.assign(this.context,{jobslog:copyData.jobslog})
        this.data = copyData;
        this.$nextTick(() => {
    ...
    public print(){
        let _this:any = this;
        _this.$print({id:'jobslog_main',popTitle:'主编辑表单'});
    }
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'JobsLogEditView视图表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {}): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'JobsLogEditView视图表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        let viewparamResult:any = Object.assign(arg,this.viewparams);
    ...
    public autoSave(opt: any = {}): void {
        ...
        if(!action){
            ...
            this.$Notice.error({ title: '错误', desc: 'JobsLogEditView视图表单'+actionName+'参数未配置' });
            return;
        }
    ...
    public remove(opt:Array<any> = [],showResultInfo?: boolean): Promise<any> {
        return new Promise((resolve: any, reject: any) => {
            if(!this.removeAction){
                this.$Notice.error({ title: '错误', desc: 'JobsLogEditView视图表单removeAction参数未配置' });
                return;
            }
            const arg: any = opt[0];
            const _this: any = this;

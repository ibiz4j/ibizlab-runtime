  ...
  <i-form style="height:100%;display:flex;flex-direction: column;justify-content: space-between">
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            <template slot="empty">
                {{$t('entities.jobslog.main_grid.nodata')}} 
            </template>
            <template v-if="!isSingleSelect">
                <el-table-column align="center" type='selection' :width="checkboxColWidth"></el-table-column>
            </template>
            <template v-if="getColumnState('id')">
                <el-table-column show-overflow-tooltip :prop="'id'" :label="$t('entities.jobslog.main_grid.columns.id')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.jobslog.main_grid.columns.id')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('job_id')">
                <el-table-column show-overflow-tooltip :prop="'job_id'" :label="$t('entities.jobslog.main_grid.columns.job_id')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.jobslog.main_grid.columns.job_id')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('handler')">
                <el-table-column show-overflow-tooltip :prop="'handler'" :label="$t('entities.jobslog.main_grid.columns.handler')" :width="250"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.jobslog.main_grid.columns.handler')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('address')">
                <el-table-column show-overflow-tooltip :prop="'address'" :label="$t('entities.jobslog.main_grid.columns.address')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.jobslog.main_grid.columns.address')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('trigger_code')">
                <el-table-column show-overflow-tooltip :prop="'trigger_code'" :label="$t('entities.jobslog.main_grid.columns.trigger_code')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.jobslog.main_grid.columns.trigger_code')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.trigger_code}}</span>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('trigger_type')">
                <el-table-column show-overflow-tooltip :prop="'trigger_type'" :label="$t('entities.jobslog.main_grid.columns.trigger_type')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.jobslog.main_grid.columns.trigger_type')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('fail_retry_count')">
                <el-table-column show-overflow-tooltip :prop="'fail_retry_count'" :label="$t('entities.jobslog.main_grid.columns.fail_retry_count')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.jobslog.main_grid.columns.fail_retry_count')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.fail_retry_count}}</span>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('create_time')">
                <el-table-column show-overflow-tooltip :prop="'create_time'" :label="$t('entities.jobslog.main_grid.columns.create_time')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.jobslog.main_grid.columns.create_time')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof MainBase
     */  
    public majorInfoColName:string = "handler";
    ...
    */
    ...
    public rules(){
        return {
            srfkey: [
                { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: `${this.$t('entities.jobslog.main_grid.columns.srfkey')}${this.$t('app.commonWords.valueNotEmpty')}`, trigger: 'change' },
                { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: `${this.$t('entities.jobslog.main_grid.columns.srfkey')}${this.$t('app.commonWords.valueNotEmpty')}`, trigger: 'blur' },
            ],
        }
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'JobsLogGridView'+(this.$t('app.gridpage.notConfig.fetchAction') as string) });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'JobsLogGridView'+(this.$t('app.gridpage.notConfig.removeAction') as string) });
            return;
        }
        let _datas:any[] = [];
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ jobslog: _keys }),Object.assign({ jobslog: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
        ...
        const doExport = async (_data:any) => {
            ...
            this.$export.exportExcel().then((excel:any)=>{
                excel.export_json_to_excel({
                  ...
                  data, //具体数据 必填
                  filename: "任务调度日志"+(this.$t('app.gridpage.grid') as string), //非必填
                  autoWidth: true, //非必填
                  bookType: "xlsx" //非必填
                });
            }); 
    ...
    */
    public arraySpanMethod({row, column, rowIndex, columnIndex} : any) {
        let allColumns:Array<any> = ['id','job_id','handler','address','trigger_code','trigger_type','fail_retry_count','create_time'];
        if(row && row.children) {
    ...
    public setColState() {
        ...
        Object.assign(viewParams,{utilServiceName: 'grid_dynaconfig', modelid: 'ibzrt_web_jobsloggridview_grid_main'});
    ...
    public saveDynaConfig() {
        const viewParams: any = Util.deepCopy(this.viewparams);
        Object.assign(viewParams,{utilServiceName: 'grid_dynaconfig', modelid: 'ibzrt_web_jobsloggridview_grid_main', model: this.allColumns});
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'JobsLogGridView'+(this.$t('app.gridpage.notConfig.createAction') as string) });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'JobsLogGridView'+(this.$t('app.gridpage.notConfig.updateAction') as string) });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.jobslog){
            ...
            errorItems.forEach((item: any, index: number) => {
                if(errorMessage[index] && errorMessage[index].data) {
                    if(Object.is(errorMessage[index].data.errorKey, 'DupCheck')) {
                        ...
                        let name: string = this.service.getNameByProp("id");
                        if(name){
                            let desc: any = this.allColumns.find((column: any) =>{
                                return Object.is(column.name, name);
                            });
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'JobsLogGridView'+(this.$t('app.gridpage.notConfig.loaddraftAction') as string) });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.jobslog,args.row.jobslog);
        });

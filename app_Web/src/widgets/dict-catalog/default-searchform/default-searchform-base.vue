  ...
  <row>
    <i-col span="20" class="form-content">
      <row>
              ...
              <input-box v-model="data.n_ccode_like"  @enter="onEnter($event)"    :disabled="detailsModel.n_ccode_like.disabled" type='text'  style=""></input-box>
          </app-form-item>
          ...
          <i-col v-show="detailsModel.n_cgroup_like.visible" :style="{}"  :sm="{ span: 24, offset: 0 }" :md="{ span: 8, offset: 0 }" :lg="{ span: 8, offset: 0 }" :xl="{ span: 8, offset: 0 }">
              ...
              <input-box v-model="data.n_cgroup_like"  @enter="onEnter($event)"    :disabled="detailsModel.n_cgroup_like.disabled" type='text'  style=""></input-box>
          </app-form-item>
...
, 
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'DictCatalogPickupGridView视图搜索表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {},mode?:string): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'DictCatalogPickupGridView视图搜索表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        Object.assign(arg,{viewparams:this.viewparams});

...
<i-col v-show="detailsModel.open_type.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-item name='open_type' :itemRules="this.rules().open_type" class='' :caption="$t('entities.sysopenaccess.main_form.details.open_type')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.open_type.error" :isEmptyCaption="false" labelPos="LEFT">
...
<i-col v-show="detailsModel.access_key.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-item name='access_key' :itemRules="this.rules().access_key" class='' :caption="$t('entities.sysopenaccess.main_form.details.access_key')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.access_key.error" :isEmptyCaption="false" labelPos="LEFT">
    <input-box 
...
<i-col v-show="detailsModel.region_id.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-item name='region_id' :itemRules="this.rules().region_id" class='' :caption="$t('entities.sysopenaccess.main_form.details.region_id')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.region_id.error" :isEmptyCaption="false" labelPos="LEFT">
    <input-box 
...
export default class MainBase extends Vue implements ControlInterface {
    ...
    public detailsModel: any = {
        group1: new FormGroupPanelModel({ caption: '第三方认证平台基本信息', detailType: 'GROUPPANEL', name: 'group1', visible: true, isShowCaption: false, form: this, isControlledContent: false , uiActionGroup: { caption: '', langbase: 'entities.sysopenaccess.main_form', extractMode: 'ITEM', details: [] }, isManageContainer: false, showMoreModeItems: []  })
, 
...
, 
    ...
    public afterCreated(){
        if(this.isautoload){
            this.autoLoad({srfkey:this.context.sysopenaccess});
        }
        if (this.viewState) {
    ...
    public print(){
        let _this:any = this;
        _this.$print({id:'sysopenaccess_main',popTitle:'主编辑表单'});
    }
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'SysOpenAccessEditView' + (this.$t('app.formpage.notconfig.loadaction') as string) });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {}): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'SysOpenAccessEditView' + (this.$t('app.formpage.notconfig.loaddraftaction') as string) });
            return;
        }
        const arg: any = { ...opt } ;
        let viewparamResult:any = Object.assign(arg,this.viewparams);
    ...
    public autoSave(opt: any = {}): void {
        ...
        if(!action){
            ...
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'SysOpenAccessEditView' + (this.$t('app.formpage.notconfig.actionname') as string) });
            return;
        }
    ...
    */
    public remove(opt:Array<any> = [],showResultInfo?: boolean): Promise<any> {
        return new Promise((resolve: any, reject: any) => {
            if(!this.removeAction){
                this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'SysOpenAccessEditView' + (this.$t('app.formpage.notconfig.removeaction') as string) });
                return;
            }
            const arg: any = opt[0];
            const _this: any = this;
    ...
    public async wfsubmit(data: any,localdata?:any): Promise<any> {
        ...
        Object.assign(arg,{viewparams:this.viewparams});
        if (!arg.sysopenaccess || Object.is(arg.sysopenaccess, '')) {
            return;
        }

  ...
  <i-form style="height:100%;display:flex;flex-direction: column;justify-content: space-between">
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            <template slot="empty">
                {{$t('entities.sysapp.main_grid.nodata')}} 
            </template>
            <template v-if="!isSingleSelect">
                <el-table-column align="center" type='selection' :width="checkboxColWidth"></el-table-column>
            </template>
            <template v-if="getColumnState('pssystemid')">
                <el-table-column show-overflow-tooltip :prop="'pssystemid'" :label="$t('entities.sysapp.main_grid.columns.pssystemid')" :width="100"  :align="'left'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.sysapp.main_grid.columns.pssystemid')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('appid')">
                <el-table-column show-overflow-tooltip :prop="'appid'" :label="$t('entities.sysapp.main_grid.columns.appid')" :width="200"  :align="'left'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.sysapp.main_grid.columns.appid')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template v-if="actualIsOpenEdit && !row.children">
            ...
            <template v-if="getColumnState('appname')">
                <el-table-column show-overflow-tooltip :prop="'appname'" :label="$t('entities.sysapp.main_grid.columns.appname')" :width="200"  :align="'left'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
            ...
            <template v-if="getColumnState('fullname')">
                <el-table-column show-overflow-tooltip :prop="'fullname'" :label="$t('entities.sysapp.main_grid.columns.fullname')" :width="250"  :align="'left'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.sysapp.main_grid.columns.fullname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template v-if="actualIsOpenEdit && !row.children">
            ...
            <template v-if="getColumnState('icon')">
                <el-table-column show-overflow-tooltip :prop="'icon'" :label="$t('entities.sysapp.main_grid.columns.icon')" :width="150"  :align="'left'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.sysapp.main_grid.columns.icon')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template v-if="actualIsOpenEdit && !row.children">
            ...
            <template v-if="getColumnState('addr')">
                <el-table-column show-overflow-tooltip :prop="'addr'" :label="$t('entities.sysapp.main_grid.columns.addr')" :width="300"  :align="'left'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.sysapp.main_grid.columns.addr')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template v-if="actualIsOpenEdit && !row.children">
            ...
            <template v-if="getColumnState('visabled')">
                <el-table-column show-overflow-tooltip :prop="'visabled'" :label="$t('entities.sysapp.main_grid.columns.visabled')" :width="100"  :align="'left'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.sysapp.main_grid.columns.visabled')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template v-if="actualIsOpenEdit && !row.children">
                            <app-form-item :error="gridItemsModel[$index][column.property].error">
                                <app-switch name='visabled' :value="row[column.property]" @change="($event)=>{row[column.property] = $event;gridEditItemChange(row, column.property, $event, $index)} " :disabled="getColumnDisabled(row,column.property)" style=""></app-switch>
            
                            </app-form-item>
                        </template>
                        <template v-if="!actualIsOpenEdit">
            <codelist :value="row.visabled" tag='YesNo' codelistType='STATIC' ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof MainBase
     */  
    public majorInfoColName:string = "appname";
    ...
    */
    ...
    public async remove(datas: any[]): Promise<any> {
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.appname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ sysapp: _keys }),Object.assign({ sysapp: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    */
    public arraySpanMethod({row, column, rowIndex, columnIndex} : any) {
        let allColumns:Array<any> = ['pssystemid','appid','appname','appgroup','apptype','fullname','icon','addr','visabled'];
        if(row && row.children) {
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.sysapp,$event.sysapp);
        });
    ...
    public setColState() {
        ...
        Object.assign(viewParams,{utilServiceName: 'grid_dynaconfig', modelid: 'ibzrt_web_sysappgridview_grid_main'});
     ...
     */
    public onColChange() {
        localStorage.setItem('sys_app_main_grid', JSON.stringify(this.allColumns));
    }
    ...
    public saveDynaConfig() {
        const viewParams: any = Util.deepCopy(this.viewparams);
        Object.assign(viewParams,{utilServiceName: 'grid_dynaconfig', modelid: 'ibzrt_web_sysappgridview_grid_main', model: this.allColumns});
            ...
            errorItems.forEach((item: any, index: number) => {
                if(errorMessage[index] && errorMessage[index].data) {
                    if(Object.is(errorMessage[index].data.errorKey, 'DupCheck')) {
                        ...
                        let name: string = this.service.getNameByProp("appid");
                        if(name){
                            let desc: any = this.allColumns.find((column: any) =>{
                                return Object.is(column.name, name);
                            });
    ...
    public updateGridEditItem(mode: string, data: any = {}, updateDetails: string[], showloading?: boolean): void {
        ...
        Object.is(tempContext, { sysapp: data.appid });

<template>
    <i-form :model="this.data" class='app-form' ref='form'  id='wfgroup_main' style="" @on-validate="formItemValidate">
...
<i-col v-show="detailsModel.groupscope.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    ...
    <input-box 
    v-model="data.groupscope"  
    @enter="onEnter($event)"  
     unit=""  
    :disabled="detailsModel.groupscope.disabled" 
    type='text' 
    style="">
</input-box>
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof MainBase
     */
    public majorMessageField: string = "groupname";
...
, 
        groupname: new FormItemModel({ caption: '组名称', detailType: 'FORMITEM', name: 'groupname', visible: true, isShowCaption: true, form: this, isControlledContent: false , required:false, disabled: false, enableCond: 3 })
, 
        groupscope: new FormItemModel({ caption: '范围', detailType: 'FORMITEM', name: 'groupscope', visible: true, isShowCaption: true, form: this, isControlledContent: false , required:false, disabled: false, enableCond: 3 })
, 
...
, 
    ...
    public afterCreated(){
        if(this.isautoload){
            this.autoLoad({srfkey:this.context.wfgroup});
        }
        if (this.viewState) {
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'WFGroupEditView' + (this.$t('app.formpage.notconfig.loadaction') as string) });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {}): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'WFGroupEditView' + (this.$t('app.formpage.notconfig.loaddraftaction') as string) });
            return;
        }
        const arg: any = { ...opt } ;
        let viewparamResult:any = Object.assign(arg,this.viewparams);
    ...
    public autoSave(opt: any = {}): void {
        ...
        if(!action){
            ...
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'WFGroupEditView' + (this.$t('app.formpage.notconfig.actionname') as string) });
            return;
        }
    ...
    public remove(opt:Array<any> = [],showResultInfo?: boolean): Promise<any> {
        return new Promise((resolve: any, reject: any) => {
            if(!this.removeAction){
                this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'WFGroupEditView' + (this.$t('app.formpage.notconfig.removeaction') as string) });
                return;
            }
            const arg: any = opt[0];
            const _this: any = this;

    ...
    <card class='view-card '  :dis-hover="true" :bordered="false">
        ...
        <div class='content-container'>
            <div style='margin-bottom: 6px;'>
                <i-input v-show="!isExpandSearchForm" v-model="query" search enter-button @on-search="onSearch($event)" class='quick-search-input' style='max-width: 400px;padding-left: 24px' placeholder="组成员名称" />
                    ...
                    <div class='toolbar-container'>
                        ...
                        <tooltip :transfer="true" :max-width="600">
                                <i-button v-show="toolBarModels.tbitem18.visabled" :disabled="toolBarModels.tbitem18.disabled" class='' v-loading:i-button @click="toolbar_click({ tag: 'tbitem18' }, $event)">
                                    ...
                                    <span class='caption'>{{$t('entities.wfmember.gridviewtoolbar_toolbar.tbitem18.caption')}}</span>
                                </i-button>
                            <div slot='content'>{{$t('entities.wfmember.gridviewtoolbar_toolbar.tbitem18.tip')}}</div>
                        </tooltip>
                    </div>
                </div>
            </div>
    ...
    public afterCreated(){
        ...
        if(_this.portletState){
            _this.portletStateEvent = _this.portletState.subscribe((res:any) =>{
                if(!Object.is(res.name,'WFMemberGridView')){
                    return;
                }

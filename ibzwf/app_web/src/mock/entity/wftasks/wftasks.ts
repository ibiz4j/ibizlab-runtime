...
// FetchDoneTask
mock.onGet(new RegExp(/^\/wftasks\/fetchdonetask$/)).reply((config: any) => {
    console.groupCollapsed("实体:wftask 方法: FetchDoneTask");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
        return [status, null];
...
// FetchDoneTask
mock.onGet(new RegExp(/^\/wftasks\/fetchdonetask(\?[\w-./?%&=,]*)*$/)).reply((config: any) => {
    console.groupCollapsed("实体:wftask 方法: FetchDoneTask");
    console.table({url:config.url, method: config.method, data:config.data});
    if(config.url.includes('page')){
        let url = config.url.split('?')[1];
...
// FetchFinishTask
mock.onGet(new RegExp(/^\/wftasks\/fetchfinishtask$/)).reply((config: any) => {
    console.groupCollapsed("实体:wftask 方法: FetchFinishTask");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
        return [status, null];
...
// FetchFinishTask
mock.onGet(new RegExp(/^\/wftasks\/fetchfinishtask(\?[\w-./?%&=,]*)*$/)).reply((config: any) => {
    console.groupCollapsed("实体:wftask 方法: FetchFinishTask");
    console.table({url:config.url, method: config.method, data:config.data});
    if(config.url.includes('page')){
        let url = config.url.split('?')[1];
...
// FetchTodoTask
mock.onGet(new RegExp(/^\/wftasks\/fetchtodotask$/)).reply((config: any) => {
    console.groupCollapsed("实体:wftask 方法: FetchTodoTask");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
        return [status, null];
...
// FetchTodoTask
mock.onGet(new RegExp(/^\/wftasks\/fetchtodotask(\?[\w-./?%&=,]*)*$/)).reply((config: any) => {
    console.groupCollapsed("实体:wftask 方法: FetchTodoTask");
    console.table({url:config.url, method: config.method, data:config.data});
    if(config.url.includes('page')){
        let url = config.url.split('?')[1];
...
// FetchToreadTask
mock.onGet(new RegExp(/^\/wftasks\/fetchtoreadtask$/)).reply((config: any) => {
    console.groupCollapsed("实体:wftask 方法: FetchToreadTask");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
        return [status, null];
...
// FetchToreadTask
mock.onGet(new RegExp(/^\/wftasks\/fetchtoreadtask(\?[\w-./?%&=,]*)*$/)).reply((config: any) => {
    console.groupCollapsed("实体:wftask 方法: FetchToreadTask");
    console.table({url:config.url, method: config.method, data:config.data});
    if(config.url.includes('page')){
        let url = config.url.split('?')[1];

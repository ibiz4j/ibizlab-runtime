...
// GetDraft
mock.onGet(new RegExp(/^\/wfsystems\/getdraft$/)).reply((config: any) => {
    console.groupCollapsed("实体:wfsystem 方法: GetDraft");
    console.table({url:config.url, method: config.method, data:config.data});
    // GetDraft
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
...
// FetchDefault
mock.onGet(new RegExp(/^\/wfsystems\/fetchdefault$/)).reply((config: any) => {
    console.groupCollapsed("实体:wfsystem 方法: FetchDefault");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
        return [status, null];
...
// FetchDefault
mock.onGet(new RegExp(/^\/wfsystems\/fetchdefault(\?[\w-./?%&=,]*)*$/)).reply((config: any) => {
    console.groupCollapsed("实体:wfsystem 方法: FetchDefault");
    console.table({url:config.url, method: config.method, data:config.data});
...
// Remove
mock.onDelete(new RegExp(/^\/wfsystems\/([a-zA-Z0-9\-\;]{1,35})$/)).reply((config: any) => {
    console.groupCollapsed("实体:wfsystem 方法: Remove");
    console.table({url:config.url, method: config.method, data:config.data});
    let status = MockAdapter.mockStatus(config);
    if (status !== 200) {
        return [status, null];
...
// Get
mock.onGet(new RegExp(/^\/wfsystems\/([a-zA-Z0-9\-\;]{1,35})$/)).reply((config: any) => {

  ...
  <row>
    <i-col span="20" class="form-content">
      <row>
              ...
              <input-box v-model="data.n_groupname_like"  @enter="onEnter($event)"    :disabled="detailsModel.n_groupname_like.disabled" type='text'  style="width:100px;"></input-box>
...
export default class DefaultBase extends Vue implements ControlInterface {
    ...
    public rules: any = {
        n_groupname_like: [
            { type: 'string', message: '组名称(文本包含(%)) 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '组名称(文本包含(%)) 值必须为字符串类型', trigger: 'blur' },
            { required: false, type: 'string', message: '组名称(文本包含(%)) 值不能为空', trigger: 'change' },
            { required: false, type: 'string', message: '组名称(文本包含(%)) 值不能为空', trigger: 'blur' },
        ],
    }
...
, 
        n_groupname_like: new FormItemModel({ caption: '组名称(文本包含(%))', detailType: 'FORMITEM', name: 'n_groupname_like', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
    };

    /**
     * 监控表单属性 n_groupname_like 值
     *
     * @param {*} newVal
     * @param {*} oldVal
     * @memberof DefaultBase
     */
    @Watch('data.n_groupname_like')
    onN_groupname_likeChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'n_groupname_like', newVal: newVal, oldVal: oldVal });
    }
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'WFGroupGridView' + (this.$t('app.searchForm.notConfig.loadAction') as string) });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {},mode?:string): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'WFGroupGridView' + (this.$t('app.searchForm.notConfig.loaddraftAction') as string) });
            return;
        }
        const arg: any = { ...opt } ;
        Object.assign(arg,{viewparams:this.viewparams});

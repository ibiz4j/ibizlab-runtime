<template>
    <i-form :model="this.data" class='app-form' ref='form'  id='wfremodel_main' style="" @on-validate="formItemValidate">
...
import WFREModelEntityService from '@/service/wfremodel/wfremodel-service';
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof MainBase
     */
    public appEntityService: WFREModelEntityService = new WFREModelEntityService({ $store: this.$store });
...
, 
    ...
    public afterCreated(){
        if(this.isautoload){
            this.autoLoad({srfkey:this.context.wfremodel});
        }
        if (this.viewState) {

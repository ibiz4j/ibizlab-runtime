<template>
    <i-form :model="this.data" class='app-form' ref='form'  id='wfprocessinstance_jump' style="" @on-validate="formItemValidate">
    <input style="display:none;" />
    <row >
            <i-col v-show="detailsModel.group1.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-group :uiService="appUIService" :data="transformData(data)" :manageContainerStatus="detailsModel.group1.manageContainerStatus"  :isManageContainer="detailsModel.group1.isManageContainer" @managecontainerclick="manageContainerClick('group1')" layoutType="TABLE_24COL" titleStyle="" class='' :uiActionGroup="detailsModel.group1.uiActionGroup" @groupuiactionclick="groupUIActionClick($event)" :caption="$t('entities.wfprocessinstance.jump_form.details.group1')" :isShowCaption="false" uiStyle="DEFAULT" :titleBarCloseMode="0" :isInfoGroupMode="false" >    
    <row>
...
<app-picker 
  ...
  :localContext ='{ }' 
  :localParam ='{ n_definitionid_eq:"%definitionid%" }' 
  ...
  editortype="" 
  :pickupView="{ viewname: 'wfprocess-node-porcess-node-pickup-view', title: $t('entities.wfprocessnode.views.porcessnodepickupview.title'), deResParameters: [], parameters: [{ pathName: 'wfprocessnodes', parameterName: 'wfprocessnode' }, { pathName: 'porcessnodepickupview', parameterName: 'porcessnodepickupview' } ], placement:'' }"
  style=""  
...
import JumpService from './jump-form-service';
...
export default class JumpBase extends Vue implements ControlInterface {
     ...
     * @memberof JumpBase
     */
    public service: JumpService = new JumpService({ $store: this.$store });
    ...
    public detailsModel: any = {
        group1: new FormGroupPanelModel({ caption: '流程实例基本信息', detailType: 'GROUPPANEL', name: 'group1', visible: true, isShowCaption: false, form: this, isControlledContent: false , uiActionGroup: { caption: '', langbase: 'entities.wfprocessinstance.jump_form', extractMode: 'ITEM', details: [] }, isManageContainer: false, showMoreModeItems: []  })
, 
...
, 
        srfkey: new FormItemModel({ caption: '实例标识', detailType: 'FORMITEM', name: 'srfkey', visible: true, isShowCaption: true, form: this, isControlledContent: false , required:false, disabled: false, enableCond: 3 })
, 
        srfmajortext: new FormItemModel({ caption: '实例名称', detailType: 'FORMITEM', name: 'srfmajortext', visible: true, isShowCaption: true, form: this, isControlledContent: false , required:false, disabled: false, enableCond: 3 })
, 
...
, 
    ...
    /**
     * 监控表单属性 taskdefinitionname 值
     *
     * @param {*} newVal
     * @param {*} oldVal
     * @memberof JumpBase
     */
    @Watch('data.taskdefinitionname')
    onTaskdefinitionnameChange(newVal: any, oldVal: any) {
     ...
     * @memberof JumpBase
     */
    @Watch('data.taskdefinitionkey')
    onTaskdefinitionkeyChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'taskdefinitionkey', newVal: newVal, oldVal: oldVal });
    }
     ...
     * @memberof JumpBase
     */
    @Watch('data.instanceid')
    onInstanceidChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'instanceid', newVal: newVal, oldVal: oldVal });
    }
    ...
    public onFormLoad(data: any = {},action:string): void {
        ...
        // 更新context的实体主键
        if(data.wfprocessinstance){
            Object.assign(this.context,{wfprocessinstance:data.wfprocessinstance})
        }
    ...
    public afterCreated(){
        if(this.isautoload){
            this.autoLoad({srfkey:this.context.wfprocessinstance});
        }
        if (this.viewState) {
    ...
    /**
     *打印
     *@memberof @memberof JumpBase
     */
    public print(){
        let _this:any = this;
        _this.$print({id:'wfprocessinstance_jump',popTitle:'流程跳转表单'});
    }
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'WFProcessInstanceActiveInstanceOptionView' + (this.$t('app.formpage.notconfig.loadaction') as string) });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {}): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'WFProcessInstanceActiveInstanceOptionView' + (this.$t('app.formpage.notconfig.loaddraftaction') as string) });
            return;
        }
        const arg: any = { ...opt } ;
        let viewparamResult:any = Object.assign(arg,this.viewparams);
    ...
    public autoSave(opt: any = {}): void {
        ...
        if(!action){
            ...
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'WFProcessInstanceActiveInstanceOptionView' + (this.$t('app.formpage.notconfig.actionname') as string) });
            return;
        }
        ...
        post.then((response: any) => {
            ...
            this.$emit('save', data);
            AppCenterService.notifyMessage({name:"WFProcessInstance",action:'appRefresh',data:data});
            this.$nextTick(() => {
    ...
    */
    public remove(opt:Array<any> = [],showResultInfo?: boolean): Promise<any> {
        return new Promise((resolve: any, reject: any) => {
            if(!this.removeAction){
                this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'WFProcessInstanceActiveInstanceOptionView' + (this.$t('app.formpage.notconfig.removeaction') as string) });
                return;
            }
            const arg: any = opt[0];
            const _this: any = this;
    ...
    public async wfsubmit(data: any,localdata?:any): Promise<any> {
        ...
        Object.assign(arg,{viewparams:this.viewparams});
        if (!arg.wfprocessinstance || Object.is(arg.wfprocessinstance, '')) {
            return;
        }
    ...
    */
    ...
    public createDefault(){                    
        if (this.data.hasOwnProperty('definitionkey')) {
            this.data['definitionkey'] = this.context['processdefinitionkey'];
        }
    ...
    public updateDefault(){                    
        if (this.data.hasOwnProperty('definitionkey') && !this.data.definitionkey) {
            this.data['definitionkey'] = this.context['processdefinitionkey'];
        }

  ...
  <i-form style="height:100%;display:flex;flex-direction: column;justify-content: space-between">
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            <template slot="empty">
                {{$t('entities.wfprocessinstance.main_grid.nodata')}} 
            </template>
            ...
            <template v-if="getColumnState('instancename')">
                <el-table-column show-overflow-tooltip :prop="'instancename'" :label="$t('entities.wfprocessinstance.main_grid.columns.instancename')" :width="250"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
            ...
            <template v-if="getColumnState('starttime')">
                <el-table-column show-overflow-tooltip :prop="'starttime'" :label="$t('entities.wfprocessinstance.main_grid.columns.starttime')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.wfprocessinstance.main_grid.columns.starttime')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <app-format-data format="YYYY-MM-DD HH:mm:ss" :data="row.starttime"></app-format-data>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('startuserid')">
                <el-table-column show-overflow-tooltip :prop="'startuserid'" :label="$t('entities.wfprocessinstance.main_grid.columns.startuserid')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.wfprocessinstance.main_grid.columns.startuserid')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
...
import WFProcessInstanceEntityService from '@/service/wfprocess-instance/wfprocess-instance-service';
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof MainBase
     */
    public appEntityService: WFProcessInstanceEntityService = new WFProcessInstanceEntityService({ $store: this.$store });
     ...
     * @memberof MainBase
     */  
    public majorInfoColName:string = "instancename";
    ...
    */
    ...
    public allColumns: any[] = [
        ...
        },
        {
            name: 'instancename',
            label: '实例名称',
            langtag: 'entities.wfprocessinstance.main_grid.columns.instancename',
            show: true,
            unit: 'PX',
            isEnableRowEdit: false,
            enableCond: 3 ,
        },
        {
            name: 'businesskey',
            label: '业务键值',
            langtag: 'entities.wfprocessinstance.main_grid.columns.businesskey',
            show: true,
            unit: 'PX',
            isEnableRowEdit: false,
            enableCond: 3 ,
        },
        {
            name: 'starttime',
            label: '启动时间',
            langtag: 'entities.wfprocessinstance.main_grid.columns.starttime',
            show: true,
            unit: 'PX',
            isEnableRowEdit: false,
            enableCond: 3 ,
        },
        {
            name: 'startuserid',
            label: '发起人标识',
            langtag: 'entities.wfprocessinstance.main_grid.columns.startuserid',
            show: true,
            unit: 'PX',
            isEnableRowEdit: false,
            enableCond: 3 ,
    ...
    public rules(){
        return {
            srfkey: [
                { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: `${this.$t('entities.wfprocessinstance.main_grid.columns.srfkey')}${this.$t('app.commonWords.valueNotEmpty')}`, trigger: 'change' },
                { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: `${this.$t('entities.wfprocessinstance.main_grid.columns.srfkey')}${this.$t('app.commonWords.valueNotEmpty')}`, trigger: 'blur' },
            ],
        }
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'WFProcessInstanceActiveInstanceGridView'+(this.$t('app.gridpage.notConfig.fetchAction') as string) });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'WFProcessInstanceActiveInstanceGridView'+(this.$t('app.gridpage.notConfig.removeAction') as string) });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.instancename;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ wfprocessinstance: _keys }),Object.assign({ wfprocessinstance: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
        ...
        const doExport = async (_data:any) => {
            ...
            this.$export.exportExcel().then((excel:any)=>{
                excel.export_json_to_excel({
                  ...
                  data, //具体数据 必填
                  filename: "流程实例"+(this.$t('app.gridpage.grid') as string), //非必填
                  autoWidth: true, //非必填
                  bookType: "xlsx" //非必填
                });
            }); 
    ...
    public afterCreated(){
        ...
        if(AppCenterService && AppCenterService.getMessageCenter()){
            this.appStateEvent = AppCenterService.getMessageCenter().subscribe(({ name, action, data }) =>{
                if(!Object.is(name,"WFProcessInstance")){
                    return;
                }
    ...
    */
    public arraySpanMethod({row, column, rowIndex, columnIndex} : any) {
        let allColumns:Array<any> = ['definitionname','instancename','businesskey','starttime','startuserid'];
        if(row && row.children) {
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.wfprocessinstance,$event.wfprocessinstance);
        });
    ...
    public setColState() {
        ...
        Object.assign(viewParams,{utilServiceName: 'grid_dynaconfig', modelid: 'ibzwf_web_wfprocessinstanceactiveinstancegridview_grid_main'});
    ...
    public getColStorage() {
        const _data: any = localStorage.getItem('wf_instance_main_grid');
        if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('wf_instance_main_grid', JSON.stringify(this.allColumns));
    }
    ...
    public saveDynaConfig() {
        const viewParams: any = Util.deepCopy(this.viewparams);
        Object.assign(viewParams,{utilServiceName: 'grid_dynaconfig', modelid: 'ibzwf_web_wfprocessinstanceactiveinstancegridview_grid_main', model: this.allColumns});
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'WFProcessInstanceActiveInstanceGridView'+(this.$t('app.gridpage.notConfig.createAction') as string) });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'WFProcessInstanceActiveInstanceGridView'+(this.$t('app.gridpage.notConfig.updateAction') as string) });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.wfprocessinstance){
                            Object.assign(this.context,{wfprocessinstance:item.wfprocessinstance});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'WFProcessInstanceActiveInstanceGridView'+(this.$t('app.gridpage.notConfig.loaddraftAction') as string) });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.wfprocessinstance,args.row.wfprocessinstance);
        });

<template>
    <i-form :model="this.data" class='app-form' ref='form'  id='wftask_main' style="">
    ...
    <row>
            ...
            <tab-pane v-show="detailsModel.formpage2.visible" name='formpage2' :index="1" tab='main' class=''  
                :label="(h) =>{
                    ...
                    $t('entities.wftask.main_form.details.formpage2')
                    ])
                }">
...
export default class MainBase extends Vue implements ControlInterface {
    ...
    return {
        ...
        srfkey: [
            { type: 'string', message: '任务标识 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '任务标识 值必须为字符串类型', trigger: 'blur' },
            { required: this.detailsModel.srfkey.required, type: 'string', message: '任务标识 值不能为空', trigger: 'change' },
            { required: this.detailsModel.srfkey.required, type: 'string', message: '任务标识 值不能为空', trigger: 'blur' },
        ],
        srfmajortext: [
            { type: 'string', message: '状态 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '状态 值必须为字符串类型', trigger: 'blur' },
            { required: this.detailsModel.srfmajortext.required, type: 'string', message: '状态 值不能为空', trigger: 'change' },
            { required: this.detailsModel.srfmajortext.required, type: 'string', message: '状态 值不能为空', trigger: 'blur' },
        ],
        srftempmode: [
            { type: 'string', message: ' 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: ' 值必须为字符串类型', trigger: 'blur' },
        ...
        taskid: [
            { type: 'string', message: '任务标识 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '任务标识 值必须为字符串类型', trigger: 'blur' },
            { required: this.detailsModel.taskid.required, type: 'string', message: '任务标识 值不能为空', trigger: 'change' },
            { required: this.detailsModel.taskid.required, type: 'string', message: '任务标识 值不能为空', trigger: 'blur' },
        ],
        }
    }
    ...
    public detailsModel: any = {
        group1: new FormGroupPanelModel({ caption: '工作流任务基本信息', detailType: 'GROUPPANEL', name: 'group1', visible: true, isShowCaption: true, form: this, isControlledContent: false , uiActionGroup: { caption: '', langbase: 'entities.wftask.main_form', extractMode: 'ITEM', details: [] }, isManageContainer: false, showMoreModeItems: []  })
, 
...
, 
        group2: new FormGroupPanelModel({ caption: '操作信息', detailType: 'GROUPPANEL', name: 'group2', visible: true, isShowCaption: true, form: this, isControlledContent: false , uiActionGroup: { caption: '', langbase: 'entities.wftask.main_form', extractMode: 'ITEM', details: [] }, isManageContainer: false, showMoreModeItems: []  })
, 
...
, 
        srfkey: new FormItemModel({ caption: '任务标识', detailType: 'FORMITEM', name: 'srfkey', visible: true, isShowCaption: true, form: this, isControlledContent: false , required:false, disabled: false, enableCond: 3 })
, 
        srfmajortext: new FormItemModel({ caption: '状态', detailType: 'FORMITEM', name: 'srfmajortext', visible: true, isShowCaption: true, form: this, isControlledContent: false , required:false, disabled: false, enableCond: 3 })
, 
...
, 
        taskid: new FormItemModel({ caption: '任务标识', detailType: 'FORMITEM', name: 'taskid', visible: true, isShowCaption: true, form: this, isControlledContent: false , required:false, disabled: false, enableCond: 3 })
, 
     ...
     */
    @Watch('data.taskid')
    onTaskidChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'taskid', newVal: newVal, oldVal: oldVal });
    }
    ...
    public print(){
        let _this:any = this;
        _this.$print({id:'wftask_main',popTitle:'主编辑表单'});
    }
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'WFTaskEditView' + (this.$t('app.formpage.notconfig.loadaction') as string) });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {}): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'WFTaskEditView' + (this.$t('app.formpage.notconfig.loaddraftaction') as string) });
            return;
        }
        const arg: any = { ...opt } ;
        let viewparamResult:any = Object.assign(arg,this.viewparams);
    ...
    public autoSave(opt: any = {}): void {
        ...
        if(!action){
            ...
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'WFTaskEditView' + (this.$t('app.formpage.notconfig.actionname') as string) });
            return;
        }
    ...
    */
    public remove(opt:Array<any> = [],showResultInfo?: boolean): Promise<any> {
        return new Promise((resolve: any, reject: any) => {
            if(!this.removeAction){
                this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'WFTaskEditView' + (this.$t('app.formpage.notconfig.removeaction') as string) });
                return;
            }
            const arg: any = opt[0];
            const _this: any = this;
    ...
    public async wfsubmit(data: any,localdata?:any): Promise<any> {
        ...
        Object.assign(arg,{viewparams:this.viewparams});
        if (!arg.wftask || Object.is(arg.wftask, '')) {
            return;
        }

    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('postcode')">
                <el-table-column show-overflow-tooltip :prop="'postcode'" :label="$t('entities.ibzpost.main_grid.columns.postcode')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.ibzpost.main_grid.columns.postcode')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('postname')">
                <el-table-column show-overflow-tooltip :prop="'postname'" :label="$t('entities.ibzpost.main_grid.columns.postname')" :width="250"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.ibzpost.main_grid.columns.postname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
...
export default class MainBase extends Vue implements ControlInterface {
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'postcode',
            label: '岗位编码',
            langtag: 'entities.ibzpost.main_grid.columns.postcode',
            show: true,
            util: 'PX',
        ...
        },
        {
            name: 'postname',
            label: '岗位名称',
            langtag: 'entities.ibzpost.main_grid.columns.postname',
            show: true,
            util: 'PX',
            isEnableRowEdit: false,
        },
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'IBZPostGridView'+(this.$t('app.gridpage.notConfig.fetchAction') as string) });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'IBZPostGridView'+(this.$t('app.gridpage.notConfig.removeAction') as string) });
            return;
        }
        let _datas:any[] = [];
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ ibzpost: _keys }),Object.assign({ ibzpost: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.ibzpost,$event.ibzpost);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('ibzpost_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('ibzpost_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'IBZPostGridView'+(this.$t('app.gridpage.notConfig.createAction') as string) });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'IBZPostGridView'+(this.$t('app.gridpage.notConfig.updateAction') as string) });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.ibzpost){
                            Object.assign(this.context,{ibzpost:item.ibzpost});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'IBZPostGridView'+(this.$t('app.gridpage.notConfig.loaddraftAction') as string) });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.ibzpost,args.row.ibzpost);
        });

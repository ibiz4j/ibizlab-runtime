<template>
    <i-form :model="this.data" class='app-form' ref='form'  id='ibzemployee_changepw' style="">
...
<i-col v-show="detailsModel.group1.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    ...
    <i-input type="password" v-model="data.oldpasswd" :disabled="detailsModel.oldpasswd.disabled" style=""></i-input>
</app-form-item>

</i-col>
<i-col v-show="detailsModel.newpasswd.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-item name='newpasswd' :itemRules="this.rules.newpasswd" class='' :caption="$t('entities.ibzemployee.changepw_form.details.newpasswd')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.newpasswd.error" :isEmptyCaption="false" labelPos="LEFT">
    <i-input type="password" v-model="data.newpasswd" :disabled="detailsModel.newpasswd.disabled" style=""></i-input>
</app-form-item>

</i-col>
<i-col v-show="detailsModel.cfmpasswd.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-item name='cfmpasswd' :itemRules="this.rules.cfmpasswd" class='' :caption="$t('entities.ibzemployee.changepw_form.details.cfmpasswd')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.cfmpasswd.error" :isEmptyCaption="false" labelPos="LEFT">
    <i-input type="password" v-model="data.cfmpasswd" :disabled="detailsModel.cfmpasswd.disabled" style=""></i-input>
</app-form-item>
...
export default class ChangePwBase extends Vue implements ControlInterface {
    ...
    public rules: any = {
        ...
        oldpasswd: [
            { type: 'string', message: '原始密码 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '原始密码 值必须为字符串类型', trigger: 'blur' },
            { required: true, type: 'string', message: '原始密码 值不能为空', trigger: 'change' },
            { required: true, type: 'string', message: '原始密码 值不能为空', trigger: 'blur' },
        ],
        newpasswd: [
            { type: 'string', message: '新密码 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '新密码 值必须为字符串类型', trigger: 'blur' },
            { required: true, type: 'string', message: '新密码 值不能为空', trigger: 'change' },
            { required: true, type: 'string', message: '新密码 值不能为空', trigger: 'blur' },
        ],
        cfmpasswd: [
            { type: 'string', message: '新密码确认 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '新密码确认 值必须为字符串类型', trigger: 'blur' },
            { required: true, type: 'string', message: '新密码确认 值不能为空', trigger: 'change' },
            { required: true, type: 'string', message: '新密码确认 值不能为空', trigger: 'blur' },
        ],
        userid: [
            { type: 'string', message: '用户标识 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '用户标识 值必须为字符串类型', trigger: 'blur' },
...
, 
        oldpasswd: new FormItemModel({ caption: '原始密码', detailType: 'FORMITEM', name: 'oldpasswd', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
        newpasswd: new FormItemModel({ caption: '新密码', detailType: 'FORMITEM', name: 'newpasswd', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
        cfmpasswd: new FormItemModel({ caption: '新密码确认', detailType: 'FORMITEM', name: 'cfmpasswd', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
...
, 
    ...
    onOldpasswdChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'oldpasswd', newVal: newVal, oldVal: oldVal });
    }
    ...
    onNewpasswdChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'newpasswd', newVal: newVal, oldVal: oldVal });
    }
    ...
    onCfmpasswdChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'cfmpasswd', newVal: newVal, oldVal: oldVal });
    }
    ...
    public print(){
        let _this:any = this;
        _this.$print({id:'ibzemployee_changepw',popTitle:'变更密码'});
    }
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'IBZEmployeeChangePwdView视图表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {}): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'IBZEmployeeChangePwdView视图表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        let viewparamResult:any = Object.assign(arg,this.viewparams);
    ...
    public autoSave(opt: any = {}): void {
        ...
        if(!action){
            ...
            this.$Notice.error({ title: '错误', desc: 'IBZEmployeeChangePwdView视图表单'+actionName+'参数未配置' });
            return;
        }
    ...
    public remove(opt:Array<any> = [],showResultInfo?: boolean): Promise<any> {
        return new Promise((resolve: any, reject: any) => {
            if(!this.removeAction){
                this.$Notice.error({ title: '错误', desc: 'IBZEmployeeChangePwdView视图表单removeAction参数未配置' });
                return;
            }
            const arg: any = opt[0];
            const _this: any = this;

<template>
    <i-form :model="this.data" class='app-form' ref='form'  id='sysdeptmember_main' style="">
    <input style="display:none;" />
    <row >
            
<i-col v-show="detailsModel.group1.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-group :uiService="appUIService" :data="transformData(data)" :manageContainerStatus="detailsModel.group1.manageContainerStatus"  :isManageContainer="detailsModel.group1.isManageContainer" @managecontainerclick="manageContainerClick('group1')" layoutType="TABLE_24COL" titleStyle="" class='' :uiActionGroup="detailsModel.group1.uiActionGroup" @groupuiactionclick="groupUIActionClick($event)" :caption="$t('entities.sysdeptmember.main_form.details.group1')" :isShowCaption="false" uiStyle="DEFAULT" :titleBarCloseMode="0" :isInfoGroupMode="false" >    
    <row>
        <i-col v-show="detailsModel.deptname.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-item name='deptname' :itemRules="this.rules().deptname" class='' :caption="$t('entities.sysdeptmember.main_form.details.deptname')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.deptname.error" :isEmptyCaption="false" labelPos="LEFT">
...
<i-col v-show="detailsModel.personname.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-item name='personname' :itemRules="this.rules().personname" class='' :caption="$t('entities.sysdeptmember.main_form.details.personname')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.personname.error" :isEmptyCaption="false" labelPos="LEFT">
...
export default class MainBase extends Vue implements ControlInterface {
    ...
    return {
        ...
        srfmajortext: [
            { type: 'string', message: '成员 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '成员 值必须为字符串类型', trigger: 'blur' },
            { required: this.detailsModel.srfmajortext.required, type: 'string', message: '成员 值不能为空', trigger: 'change' },
            { required: this.detailsModel.srfmajortext.required, type: 'string', message: '成员 值不能为空', trigger: 'blur' },
        ],
        srftempmode: [
            { type: 'string', message: ' 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: ' 值必须为字符串类型', trigger: 'blur' },
        ...
        personname: [
            { type: 'string', message: '成员 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '成员 值必须为字符串类型', trigger: 'blur' },
            { required: this.detailsModel.personname.required, type: 'string', message: '成员 值不能为空', trigger: 'change' },
            { required: this.detailsModel.personname.required, type: 'string', message: '成员 值不能为空', trigger: 'blur' },
        ],
        userid: [
        ...
        memberid: [
            { type: 'string', message: '标识 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '标识 值必须为字符串类型', trigger: 'blur' },
            { required: this.detailsModel.memberid.required, type: 'string', message: '标识 值不能为空', trigger: 'change' },
            { required: this.detailsModel.memberid.required, type: 'string', message: '标识 值不能为空', trigger: 'blur' },
        ],
        }
    }
    ...
    public detailsModel: any = {
        group1: new FormGroupPanelModel({ caption: '部门成员基本信息', detailType: 'GROUPPANEL', name: 'group1', visible: true, isShowCaption: false, form: this, isControlledContent: false , uiActionGroup: { caption: '', langbase: 'entities.sysdeptmember.main_form', extractMode: 'ITEM', details: [] }, isManageContainer: false, showMoreModeItems: []  })
, 
...
, 
        srfmajortext: new FormItemModel({ caption: '成员', detailType: 'FORMITEM', name: 'srfmajortext', visible: true, isShowCaption: true, form: this, isControlledContent: false , required:false, disabled: false, enableCond: 3 })
, 
...
, 
        deptname: new FormItemModel({ caption: '部门名称', detailType: 'FORMITEM', name: 'deptname', visible: true, isShowCaption: true, form: this, isControlledContent: false , required:false, disabled: false, enableCond: 3 })
, 
...
, 
        personname: new FormItemModel({ caption: '成员', detailType: 'FORMITEM', name: 'personname', visible: true, isShowCaption: true, form: this, isControlledContent: false , required:false, disabled: false, enableCond: 3 })
, 
...
, 
        memberid: new FormItemModel({ caption: '标识', detailType: 'FORMITEM', name: 'memberid', visible: true, isShowCaption: true, form: this, isControlledContent: false , required:false, disabled: false, enableCond: 3 })
, 
    };
     ...
     */
    @Watch('data.memberid')
    onMemberidChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'memberid', newVal: newVal, oldVal: oldVal });
    }
    ...
    public onFormLoad(data: any = {},action:string): void {
        ...
        // 更新context的实体主键
        if(data.sysdeptmember){
            Object.assign(this.context,{sysdeptmember:data.sysdeptmember})
        }
    ...
    public print(){
        let _this:any = this;
        _this.$print({id:'sysdeptmember_main',popTitle:'主编辑表单'});
    }
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'SysDeptMemberEditView' + (this.$t('app.formpage.notconfig.loadaction') as string) });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {}): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'SysDeptMemberEditView' + (this.$t('app.formpage.notconfig.loaddraftaction') as string) });
            return;
        }
        const arg: any = { ...opt } ;
        let viewparamResult:any = Object.assign(arg,this.viewparams);
    ...
    public autoSave(opt: any = {}): void {
        ...
        if(!action){
            ...
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'SysDeptMemberEditView' + (this.$t('app.formpage.notconfig.actionname') as string) });
            return;
        }
        ...
        post.then((response: any) => {
            ...
            this.$emit('save', data);
            AppCenterService.notifyMessage({name:"SysDeptMember",action:'appRefresh',data:data});
            this.$nextTick(() => {
    ...
    */
    public remove(opt:Array<any> = [],showResultInfo?: boolean): Promise<any> {
        return new Promise((resolve: any, reject: any) => {
            if(!this.removeAction){
                this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'SysDeptMemberEditView' + (this.$t('app.formpage.notconfig.removeaction') as string) });
                return;
            }
            const arg: any = opt[0];
            const _this: any = this;
    ...
    public async wfsubmit(data: any,localdata?:any): Promise<any> {
        ...
        Object.assign(arg,{viewparams:this.viewparams});
        if (!arg.sysdeptmember || Object.is(arg.sysdeptmember, '')) {
            return;
        }

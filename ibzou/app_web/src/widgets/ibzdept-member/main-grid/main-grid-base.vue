    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('deptname')">
                <el-table-column show-overflow-tooltip :prop="'deptname'" :label="$t('entities.ibzdeptmember.main_grid.columns.deptname')" :width="200"  :align="'left'" :sortable="'custom'">
            ...
            <template v-if="getColumnState('postname')">
                <el-table-column show-overflow-tooltip :prop="'postname'" :label="$t('entities.ibzdeptmember.main_grid.columns.postname')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
            ...
            <template v-if="getColumnState('personname')">
                <el-table-column show-overflow-tooltip :prop="'personname'" :label="$t('entities.ibzdeptmember.main_grid.columns.personname')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
            ...
            <template v-if="getColumnState('postid')">
                <el-table-column show-overflow-tooltip :prop="'postid'" :label="$t('entities.ibzdeptmember.main_grid.columns.postid')" :width="100"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
            ...
            <template v-if="getColumnState('userid')">
                <el-table-column show-overflow-tooltip :prop="'userid'" :label="$t('entities.ibzdeptmember.main_grid.columns.userid')" :width="100"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * 实体服务对象
     *
     * @type {IBZDeptMemberService}
     * @memberof MainBase
     */
    public appEntityService: IBZDeptMemberService = new IBZDeptMemberService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'deptname',
            label: '部门',
            langtag: 'entities.ibzdeptmember.main_grid.columns.deptname',
            show: true,
            util: 'PX',
        ...
        },
        {
            name: 'postname',
            label: '岗位名称',
            langtag: 'entities.ibzdeptmember.main_grid.columns.postname',
            show: true,
            util: 'PX',
            isEnableRowEdit: true,
        },
        {
            name: 'personname',
            label: '成员',
            langtag: 'entities.ibzdeptmember.main_grid.columns.personname',
            show: true,
            util: 'PX',
            isEnableRowEdit: true,
        },
        {
            name: 'deptid',
            label: '部门标识',
            langtag: 'entities.ibzdeptmember.main_grid.columns.deptid',
            show: false,
            util: 'PX',
            isEnableRowEdit: true,
        },
        {
            name: 'postid',
            label: '岗位标识',
            langtag: 'entities.ibzdeptmember.main_grid.columns.postid',
            show: false,
            util: 'PX',
            isEnableRowEdit: true,
        },
        {
            name: 'userid',
            label: '用户标识',
            langtag: 'entities.ibzdeptmember.main_grid.columns.userid',
            show: false,
            util: 'PX',
            isEnableRowEdit: true,
        },
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'IBZDeptMemberGridView'+(this.$t('app.gridpage.notConfig.fetchAction') as string) });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'IBZDeptMemberGridView'+(this.$t('app.gridpage.notConfig.removeAction') as string) });
            return;
        }
        let _datas:any[] = [];
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ ibzdeptmember: _keys }),Object.assign({ ibzdeptmember: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public afterCreated(){
        ...
        if(AppCenterService && AppCenterService.getMessageCenter()){
            this.appStateEvent = AppCenterService.getMessageCenter().subscribe(({ name, action, data }) =>{
                if(!Object.is(name,"IBZDeptMember")){
                    return;
                }
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.ibzdeptmember,$event.ibzdeptmember);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('ibzdeptmember_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('ibzdeptmember_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'IBZDeptMemberGridView'+(this.$t('app.gridpage.notConfig.createAction') as string) });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'IBZDeptMemberGridView'+(this.$t('app.gridpage.notConfig.updateAction') as string) });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'IBZDeptMemberGridView'+(this.$t('app.gridpage.notConfig.loaddraftAction') as string) });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.ibzdeptmember,args.row.ibzdeptmember);
        });

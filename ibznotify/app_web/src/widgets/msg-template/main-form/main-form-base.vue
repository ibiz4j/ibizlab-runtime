<template>
    ...
    <app-form-item name='accessname' :itemRules="this.rules().accessname" class='' :caption="$t('entities.msgtemplate.main_form.details.accessname')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.accessname.error" :isEmptyCaption="false" labelPos="LEFT">
...
<app-picker 
  ...
  :service="service"
  :acParams="{ serviceName: 'MsgOpenAccessService', interfaceName: 'FetchDefault'}"
  valueitem='accessid' 
  :value="data.accessname" 
  editortype="" 
  :pickupView="{ viewname: 'msg-open-access-pickup-view', title: $t('entities.msgopenaccess.views.pickupview.title'), deResParameters: [], parameters: [{ pathName: 'msgopenaccesses', parameterName: 'msgopenaccess' }, { pathName: 'pickupview', parameterName: 'pickupview' } ], placement:'' }"
  style=""  
...
<i-col v-show="detailsModel.template_type.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-item name='template_type' :itemRules="this.rules().template_type" class='' :caption="$t('entities.msgtemplate.main_form.details.template_type')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.template_type.error" :isEmptyCaption="false" labelPos="LEFT">
    
<app-radio-group 
  v-model="data.template_type"
  :data="data" 
  :context="context"
  :viewparams="viewparams"   
  :disabled="detailsModel.template_type.disabled"  
  name="template_type" 
  :localContext ='{ }' 
  :localParam ='{ }' 
  tag='TemplateType' 
...
<i-col v-show="detailsModel.template_name.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-item name='template_name' :itemRules="this.rules().template_name" class='' :caption="$t('entities.msgtemplate.main_form.details.template_name')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.template_name.error" :isEmptyCaption="false" labelPos="LEFT">
    <input-box 
    v-model="data.template_name"  
    @enter="onEnter($event)"  
     unit=""  
    :disabled="detailsModel.template_name.disabled" 
    type='text' 
    style="">
</input-box>
...
<i-col v-show="detailsModel.template_id.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-item name='template_id' :itemRules="this.rules().template_id" class='' :caption="$t('entities.msgtemplate.main_form.details.template_id')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.template_id.error" :isEmptyCaption="false" labelPos="LEFT">
    <input-box 
    v-model="data.template_id"  
    @enter="onEnter($event)"  
     unit=""  
    :disabled="detailsModel.template_id.disabled" 
    type='text' 
    style="">
</input-box>
...
<i-col v-show="detailsModel.grouppanel1.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-group :uiService="appUIService" :data="transformData(data)" :manageContainerStatus="detailsModel.grouppanel1.manageContainerStatus"  :isManageContainer="detailsModel.grouppanel1.isManageContainer" @managecontainerclick="manageContainerClick('grouppanel1')" layoutType="TABLE_24COL" titleStyle="" class='' :uiActionGroup="detailsModel.grouppanel1.uiActionGroup" @groupuiactionclick="groupUIActionClick($event)" :caption="$t('entities.msgtemplate.main_form.details.grouppanel1')" :isShowCaption="false" uiStyle="DEFAULT" :titleBarCloseMode="0" :isInfoGroupMode="false" >    
    <row>
        <i-col v-show="detailsModel.content.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-item name='content' :itemRules="this.rules().content" class='' :caption="$t('entities.msgtemplate.main_form.details.content')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.content.error" :isEmptyCaption="false" labelPos="LEFT">
    <input-box v-model="data.content" :textareaId="this.$util.createUUID()"  :disabled="detailsModel.content.disabled" type='textarea' textareaStyle="height:200px;" ></input-box>

</app-form-item>

</i-col>
<i-col v-show="detailsModel.template_url.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
    <app-form-item name='template_url' :itemRules="this.rules().template_url" class='' :caption="$t('entities.msgtemplate.main_form.details.template_url')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.template_url.error" :isEmptyCaption="false" labelPos="LEFT">
    <input-box v-model="data.template_url" :textareaId="this.$util.createUUID()"  :disabled="detailsModel.template_url.disabled" type='textarea' textareaStyle="height:200px;" ></input-box>
...
export default class MainBase extends Vue implements ControlInterface {
    ...
    return {
        ...
        srfkey: [
            { type: 'string', message: '模板标识 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '模板标识 值必须为字符串类型', trigger: 'blur' },
            { required: this.detailsModel.srfkey.required, type: 'string', message: '模板标识 值不能为空', trigger: 'change' },
            { required: this.detailsModel.srfkey.required, type: 'string', message: '模板标识 值不能为空', trigger: 'blur' },
        ],
        srfmajortext: [
            { type: 'string', message: '模板标题 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '模板标题 值必须为字符串类型', trigger: 'blur' },
            { required: this.detailsModel.srfmajortext.required, type: 'string', message: '模板标题 值不能为空', trigger: 'change' },
            { required: this.detailsModel.srfmajortext.required, type: 'string', message: '模板标题 值不能为空', trigger: 'blur' },
        ],
        srftempmode: [
            { type: 'string', message: ' 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: ' 值必须为字符串类型', trigger: 'blur' },
        ...
        accessname: [
            ...
            { required: this.detailsModel.accessname.required, type: 'string', message: '接入平台 值不能为空', trigger: 'change' },
            { required: this.detailsModel.accessname.required, type: 'string', message: '接入平台 值不能为空', trigger: 'blur' },
        ],
        open_type: [
            { type: 'string', message: '接入平台类型 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '接入平台类型 值必须为字符串类型', trigger: 'blur' },
            { required: this.detailsModel.open_type.required, type: 'string', message: '接入平台类型 值不能为空', trigger: 'change' },
            { required: this.detailsModel.open_type.required, type: 'string', message: '接入平台类型 值不能为空', trigger: 'blur' },
        ],
        template_type: [
            { type: 'string', message: '模板类型 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '模板类型 值必须为字符串类型', trigger: 'blur' },
            { required: this.detailsModel.template_type.required, type: 'string', message: '模板类型 值不能为空', trigger: 'change' },
            { required: this.detailsModel.template_type.required, type: 'string', message: '模板类型 值不能为空', trigger: 'blur' },
        ],
        template_name: [
            { type: 'string', message: '模板标题 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '模板标题 值必须为字符串类型', trigger: 'blur' },
            { required: this.detailsModel.template_name.required, type: 'string', message: '模板标题 值不能为空', trigger: 'change' },
            { required: this.detailsModel.template_name.required, type: 'string', message: '模板标题 值不能为空', trigger: 'blur' },
        ],
        template_id: [
            { type: 'string', message: '模板标识 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '模板标识 值必须为字符串类型', trigger: 'blur' },
            { required: this.detailsModel.template_id.required, type: 'string', message: '模板标识 值不能为空', trigger: 'change' },
            { required: this.detailsModel.template_id.required, type: 'string', message: '模板标识 值不能为空', trigger: 'blur' },
        ],
        content: [
            { type: 'string', message: '模板内容 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '模板内容 值必须为字符串类型', trigger: 'blur' },
            { required: this.detailsModel.content.required, type: 'string', message: '模板内容 值不能为空', trigger: 'change' },
            { required: this.detailsModel.content.required, type: 'string', message: '模板内容 值不能为空', trigger: 'blur' },
        ],
        template_url: [
            { type: 'string', message: 'URL 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: 'URL 值必须为字符串类型', trigger: 'blur' },
            { required: this.detailsModel.template_url.required, type: 'string', message: 'URL 值不能为空', trigger: 'change' },
            { required: this.detailsModel.template_url.required, type: 'string', message: 'URL 值不能为空', trigger: 'blur' },
        ],
        tid: [
            { type: 'string', message: '模板标识 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '模板标识 值必须为字符串类型', trigger: 'blur' },
            { required: this.detailsModel.tid.required, type: 'string', message: '模板标识 值不能为空', trigger: 'change' },
            { required: this.detailsModel.tid.required, type: 'string', message: '模板标识 值不能为空', trigger: 'blur' },
        ],
        }
    }
    ...
    public detailsModel: any = {
        group1: new FormGroupPanelModel({ caption: '消息模板基本信息', detailType: 'GROUPPANEL', name: 'group1', visible: true, isShowCaption: false, form: this, isControlledContent: false , uiActionGroup: { caption: '', langbase: 'entities.msgtemplate.main_form', extractMode: 'ITEM', details: [] }, isManageContainer: false, showMoreModeItems: []  })
, 
        grouppanel1: new FormGroupPanelModel({ caption: '分组面板', detailType: 'GROUPPANEL', name: 'grouppanel1', visible: true, isShowCaption: false, form: this, isControlledContent: false , uiActionGroup: { caption: '', langbase: 'entities.msgtemplate.main_form', extractMode: 'ITEM', details: [] }, isManageContainer: false, showMoreModeItems: []  })
, 
...
, 
        srfkey: new FormItemModel({ caption: '模板标识', detailType: 'FORMITEM', name: 'srfkey', visible: true, isShowCaption: true, form: this, isControlledContent: false , required:false, disabled: false, enableCond: 3 })
, 
        srfmajortext: new FormItemModel({ caption: '模板标题', detailType: 'FORMITEM', name: 'srfmajortext', visible: true, isShowCaption: true, form: this, isControlledContent: false , required:false, disabled: false, enableCond: 3 })
, 
...
, 
        accessname: new FormItemModel({ caption: '接入平台', detailType: 'FORMITEM', name: 'accessname', visible: true, isShowCaption: true, form: this, isControlledContent: false , required:false, disabled: false, enableCond: 3 })
, 
        open_type: new FormItemModel({ caption: '接入平台类型', detailType: 'FORMITEM', name: 'open_type', visible: true, isShowCaption: true, form: this, isControlledContent: false , required:false, disabled: false, enableCond: 3 })
, 
        template_type: new FormItemModel({ caption: '模板类型', detailType: 'FORMITEM', name: 'template_type', visible: true, isShowCaption: true, form: this, isControlledContent: false , required:true, disabled: false, enableCond: 3 })
, 
        template_name: new FormItemModel({ caption: '模板标题', detailType: 'FORMITEM', name: 'template_name', visible: true, isShowCaption: true, form: this, isControlledContent: false , required:false, disabled: false, enableCond: 3 })
, 
        template_id: new FormItemModel({ caption: '模板标识', detailType: 'FORMITEM', name: 'template_id', visible: true, isShowCaption: true, form: this, isControlledContent: false , required:false, disabled: false, enableCond: 3 })
, 
        content: new FormItemModel({ caption: '模板内容', detailType: 'FORMITEM', name: 'content', visible: true, isShowCaption: true, form: this, isControlledContent: false , required:false, disabled: false, enableCond: 3 })
, 
        template_url: new FormItemModel({ caption: 'URL', detailType: 'FORMITEM', name: 'template_url', visible: true, isShowCaption: true, form: this, isControlledContent: false , required:false, disabled: false, enableCond: 3 })
, 
        tid: new FormItemModel({ caption: '模板标识', detailType: 'FORMITEM', name: 'tid', visible: true, isShowCaption: true, form: this, isControlledContent: false , required:false, disabled: false, enableCond: 3 })
, 
    };
     ...
     * @memberof MainBase
     */
    @Watch('data.accessname')
    onAccessnameChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'accessname', newVal: newVal, oldVal: oldVal });
    }
     ...
     * @memberof MainBase
     */
    @Watch('data.open_type')
    onOpen_typeChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'open_type', newVal: newVal, oldVal: oldVal });
    }

    /**
     * 监控表单属性 template_type 值
     *
     * @param {*} newVal
     * @param {*} oldVal
     * @memberof MainBase
     */
    @Watch('data.template_type')
    onTemplate_typeChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'template_type', newVal: newVal, oldVal: oldVal });
    }

    /**
     * 监控表单属性 template_name 值
     *
     * @param {*} newVal
     * @param {*} oldVal
     * @memberof MainBase
     */
    @Watch('data.template_name')
    onTemplate_nameChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'template_name', newVal: newVal, oldVal: oldVal });
    }

    /**
     * 监控表单属性 template_id 值
     *
     * @param {*} newVal
     * @param {*} oldVal
     * @memberof MainBase
     */
    @Watch('data.template_id')
    onTemplate_idChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'template_id', newVal: newVal, oldVal: oldVal });
    }
    ...
    /**
     * 监控表单属性 template_url 值
     *
     * @param {*} newVal
     * @param {*} oldVal
     * @memberof MainBase
     */
    @Watch('data.template_url')
    onTemplate_urlChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'template_url', newVal: newVal, oldVal: oldVal });
    }
     ...
     */
    @Watch('data.tid')
    onTidChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'tid', newVal: newVal, oldVal: oldVal });
    }
    ...
    public print(){
        let _this:any = this;
        _this.$print({id:'msgtemplate_main',popTitle:'主编辑表单'});
    }
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'MsgTemplateEditView' + (this.$t('app.formpage.notconfig.loadaction') as string) });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {}): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'MsgTemplateEditView' + (this.$t('app.formpage.notconfig.loaddraftaction') as string) });
            return;
        }
        const arg: any = { ...opt } ;
        let viewparamResult:any = Object.assign(arg,this.viewparams);
    ...
    public autoSave(opt: any = {}): void {
        ...
        if(!action){
            ...
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'MsgTemplateEditView' + (this.$t('app.formpage.notconfig.actionname') as string) });
            return;
        }
    ...
    */
    public remove(opt:Array<any> = [],showResultInfo?: boolean): Promise<any> {
        return new Promise((resolve: any, reject: any) => {
            if(!this.removeAction){
                this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'MsgTemplateEditView' + (this.$t('app.formpage.notconfig.removeaction') as string) });
                return;
            }
            const arg: any = opt[0];
            const _this: any = this;
    ...
    public async wfsubmit(data: any,localdata?:any): Promise<any> {
        ...
        Object.assign(arg,{viewparams:this.viewparams});
        if (!arg.msgtemplate || Object.is(arg.msgtemplate, '')) {
            return;
        }

<template>
    <i-form :model="this.data" class='app-form' ref='form'  id='msguseraccount_main' style="" @on-validate="formItemValidate">
...
<i-col v-show="detailsModel.identity_type.visible" :style="{}"  :md="{ span: 12, offset: 0 }" :lg="{ span: 12, offset: 0 }">
    <app-form-item name='identity_type' :itemRules="this.rules().identity_type" class='' :caption="$t('entities.msguseraccount.main_form.details.identity_type')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.identity_type.error" :isEmptyCaption="false" labelPos="LEFT">
...
<i-col v-show="detailsModel.identifier.visible" :style="{}"  :md="{ span: 12, offset: 0 }" :lg="{ span: 12, offset: 0 }">
    <app-form-item name='identifier' :itemRules="this.rules().identifier" class='' :caption="$t('entities.msguseraccount.main_form.details.identifier')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.identifier.error" :isEmptyCaption="false" labelPos="LEFT">
    <input-box 
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof MainBase
     */
    public majorMessageField: string = "identifier";
...
, 
        srfmajortext: new FormItemModel({ caption: '认证标识', detailType: 'FORMITEM', name: 'srfmajortext', visible: true, isShowCaption: true, form: this, isControlledContent: false , required:false, disabled: false, enableCond: 3 })
, 
...
, 
    ...
    public afterCreated(){
        if(this.isautoload){
            this.autoLoad({srfkey:this.context.msguseraccount});
        }
        if (this.viewState) {
    ...
    public print(){
        let _this:any = this;
        _this.$print({id:'msguseraccount_main',popTitle:'主编辑表单'});
    }
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'MsgUserAccountEditView' + (this.$t('app.formpage.notconfig.loadaction') as string) });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {}): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'MsgUserAccountEditView' + (this.$t('app.formpage.notconfig.loaddraftaction') as string) });
            return;
        }
        const arg: any = { ...opt } ;
        let viewparamResult:any = Object.assign(arg,this.viewparams);
    ...
    public autoSave(opt: any = {}): void {
        ...
        if(!action){
            ...
            this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'MsgUserAccountEditView' + (this.$t('app.formpage.notconfig.actionname') as string) });
            return;
        }
    ...
    */
    public remove(opt:Array<any> = [],showResultInfo?: boolean): Promise<any> {
        return new Promise((resolve: any, reject: any) => {
            if(!this.removeAction){
                this.$Notice.error({ title: (this.$t('app.commonWords.wrong') as string), desc: 'MsgUserAccountEditView' + (this.$t('app.formpage.notconfig.removeaction') as string) });
                return;
            }
            const arg: any = opt[0];
            const _this: any = this;
    ...
    public async wfsubmit(data: any,localdata?:any): Promise<any> {
        ...
        Object.assign(arg,{viewparams:this.viewparams});
        if (!arg.msguseraccount || Object.is(arg.msguseraccount, '')) {
            return;
        }

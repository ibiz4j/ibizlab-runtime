...
<div class="app-app-menu">
    <el-menu
      ...
      <template v-if="Object.is(mode,'horizontal')">
                ...
                <template v-if="item0.items && Array.isArray(item0.items) && item0.items.length > 0">
                    <el-submenu v-show="!item0.hidden" :index="item0.name" :popper-class="popperClass" :key="item0.id" :class="item0.textcls">
                        <template slot='title'>
                            ...
                            <span class='text' :title="$t('app.menus.notifyindexview.' + item0.name.toLowerCase())">{{$t('app.menus.notifyindexview.' + item0.name.toLowerCase())}}</span>
                        </template>
                            ...
                            <template v-if="item1.items && Array.isArray(item1.items) && item1.items.length > 0">
                                <el-submenu v-show="!item1.hidden" :index="item1.name" :popper-class="popperClass" :key="item1.id" :class="item1.textcls">
                                    <template slot='title'>
                                        ...
                                        <span class='text' :title="$t('app.menus.notifyindexview.' + item1.name.toLowerCase())">{{$t('app.menus.notifyindexview.' + item1.name.toLowerCase())}}</span>
                                    </template>
                                    <template v-for="item2 in item1.items">
                                        <template v-if="item2.type =='MENUITEM'">
                                            <el-menu-item v-show="!item2.hidden" :index="item2.name" :key="item2.id" :class="item2.textcls">
                                                ...
                                                <template slot="title">
                                                    <span class="text" :title="$t('app.menus.notifyindexview.' + item2.name.toLowerCase())">{{$t('app.menus.notifyindexview.' + item2.name.toLowerCase())}}</span>
        ...
        <app-menu-item v-else :isCollapse="isCollapse" :menus="menus" :ctrlName="'notifyindexview'" :isFirst="true" :counterdata="counterdata" :popper-class="popperClass"></app-menu-item>
...
import NotifyIndexViewService from './notify-index-view-appmenu-service';
...
export default class NotifyIndexViewBase extends Vue implements ControlInterface {
     ...
     * 建构部件服务对象
     *
     * @type {NotifyIndexViewService}
     * @memberof NotifyIndexViewBase
     */
    public service: NotifyIndexViewService = new NotifyIndexViewService({ $store: this.$store });
     ...
     *
     * @public
     * @type {NotifyIndexViewModel}
     * @memberof NotifyIndexViewBase
     */
    public menuMode: NotifyIndexViewModel = new NotifyIndexViewModel();
...
<style lang='less'>
@import './notify-index-view-appmenu.less';
</style>